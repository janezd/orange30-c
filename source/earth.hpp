...
class TEarthLearner : public TLearner {
public:
	__REGISTER_CLASS(EarthLearner);
	...
	double threshold; //P Forward step threshold.
	bool prune; //P Prune terms (do backward pass).
	double fast_beta; //P Fast beta (aging coefficient).
	double trace; //P Execution trace (for debugging only).
...
class TEarthClassifier: public TClassifier {
public:
	__REGISTER_CLASS(EarthClassifier);

	TEarthClassifier();
	TEarthClassifier(PDomain const &domain,
        bool *best_set, int * irs, double *cuts, double *betas,
        int num_preds, int num_responses, int num_terms, int max_terms);
	...
	void save_model(TByteStream & buffer);
	void load_model(TByteStream & buffer);
...
private:
	...
	void init_members();
	double* to_xvector(TExample const *const);
...
public:
    static TOrange *__new__(PyTypeObject *type, PyObject *args, PyObject *kw) PYDOC("(buffer); unpickle the classifier");
    PyObject *__getnewargs__() PYARGS(METH_NOARGS, "(); prepare arguments for unpickling");
    PyObject *formatEarth() PYARGS(METH_NOARGS, "(); format the earth");
};
