...
static struct PyModuleDef statc_module_def = {
...
STATC_API PyObject *PyInit_statc()
{ if (   ((PyExc_StatcKernel = makeExceptionClass("statc.KernelException", "an error occurred in statc's C++ code")) == NULL)
  ...
  PyObject *me = PyModule_Create(&statc_module_def);

  PyObject *pdm = PyModule_New("pointDistribution");
  PyModule_AddObject(pdm, "Minimal", PyLong_FromLong(DISTRIBUTE_MINIMAL));
  PyModule_AddObject(pdm, "Factor", PyLong_FromLong(DISTRIBUTE_FACTOR));
  PyModule_AddObject(pdm, "Fixed", PyLong_FromLong(DISTRIBUTE_FIXED));
  PyModule_AddObject(pdm, "Uniform", PyLong_FromLong(DISTRIBUTE_UNIFORM));
  PyModule_AddObject(pdm, "Maximal", PyLong_FromLong(DISTRIBUTE_MAXIMAL));
...
{ PyObject *pyn=PyNumber_Long(pyo);
  if (!pyn)
    PYERROR(PyExc_TypeError, "invalid number", false);
  dd=int(PyLong_AsLong(pyn));
  Py_DECREF(pyn);
  return true;
}
...
{ PyObject *pylist=PyList_New(ilist.size());
  ...
  const_ITERATE(vector<int>, fi, ilist)
    PyList_SetItem(pylist, i++, PyLong_FromLong(*fi));
  return pylist;
}
  ...
  { if (cb)
      if (cb==Py_None)
        ...
        if (!PyCallable_Check(cb))
          raiseError(PyExc_TypeError, "Callback: non-callable callback function");
...
public:
  ...
  { if (!x.pyobject)
      raiseError(PyExc_TypeError, "BoolUnaryCallable: invalid object");
...
public:
    ...
    bool operator()(const PyWrapper &x) const
    { if (!x.pyobject)
        raiseError(TypeError, "IsTrueCallback: invalid object");
...
public:
    ...
    bool operator()(const PyWrapper &x, const PyWrapper &y) const
    { if (!x.pyobject || !y.pyobject)
        raiseError(PyExc_TypeError, "CompareCallback: invalid objects");
