...
public:
  ...
  bool operator < (const PyWrapper &other) const
    { return PyObject_RichCompare(pyobject, other.pyobject, Py_LT) != 0; }

  bool operator <= (const PyWrapper &other) const
    { return PyObject_RichCompare(pyobject, other.pyobject, Py_LE) != 0; }

  bool operator > (const PyWrapper &other) const
    { return PyObject_RichCompare(pyobject, other.pyobject, Py_GT) != 0; }

  bool operator >= (const PyWrapper &other) const
    { return PyObject_RichCompare(pyobject, other.pyobject, Py_GE) != 0; }

  bool operator == (const PyWrapper &other) const
    { return PyObject_RichCompare(pyobject, other.pyobject, Py_EQ) != 0; }
  ...
  PyWrapper operator / (const PyWrapper &other) const
    { if (!pyobject || !other.pyobject)
      ...
      return PyWrapper(PyNumber_TrueDivide(pyobject, other));
    }
  ...
  PyWrapper &operator /= (const PyWrapper &other)
    { if (!pyobject || !other.pyobject) {
      ...
      PyObject *res=PyNumber_TrueDivide(pyobject, other);
      inplace(res);
      Py_XDECREF(res);
      return *this;
    }
  ...
  PyWrapper &operator ++ ()
    { if (!pyobject)
      ...
      PyObject *res=PyNumber_Add(pyobject, PyLong_FromLong(1));
      inplace(res);
      Py_XDECREF(res);
      return *this;
    }
...
{ if (!x.pyobject)
  ...
  PyObject *pyn=PyNumber_Long(x.pyobject);
  PyWrapper::checkForError();

  return int(PyLong_AsLong(pyn));
}
