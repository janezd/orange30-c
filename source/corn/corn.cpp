...
static struct PyModuleDef corn_module_def = {
...
{ if (   ((PyExc_CornKernel = makeExceptionClass("corn.KernelException", "An error occurred in corn's C++ code")) == NULL)
  ...
  return PyModule_Create(&corn_module_def);
}
...
{ 
  ...
  int res = (int)PyLong_AsLong(temp);
  Py_DECREF(temp);
  return res;
}
...
{ 
  ...
  try {
    ...
    for(i = 0, e = PyList_Size(temp); i<e; i++) {
      ...
      if (!cp || !PyLong_Check(cp))
        throw CornException("error in attribute 'classes'");
      classes.push_back((int)PyLong_AsLong(cp));
    }
    Py_DECREF(temp);
    temp = PYNULL;
...
{ 
  ...
  baseClass = temp ? PyLong_AsLong(temp) : -1;
  Py_DECREF(temp);
...
PyObject *computeCDTList(vector<TCummulativeROC> &cummlists)
{
  ...
  try {
    ...
    ITERATE(vector<TCDT>, cdti, cdts) {
      ...
      PyObject *PyCDT = PyObject_Call(CDTType, inargs, indict);
      Py_DECREF(inargs);
      Py_DECREF(indict);
...
public:
  ...
  int operator()(PyObject *obj1, PyObject *obj2)
    { PyObject *args = Py_BuildValue("OO", obj1, obj2);
      ...
      bool res = PyLong_AsLong(result)<0;
      Py_DECREF(result);
      return res;
    }
};
