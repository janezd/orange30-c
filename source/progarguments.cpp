    ...
    if ((*si)[0]=='-') {
      ...
      if (possibleOptions.find(option)==possibleOptions.end())
        ...
        if (si==optionsList.end())
          raiseError(PyExc_ValueError, "missing parameter for option '%s'", option.c_str());
        else
      ...
      if (ei==ee) direct.push_back(*(si++));
        ...
        if (possibleOptions.find(option)==possibleOptions.end())
          ...
          raiseError(PyExc_ValueError, "option '%s' expects no arguments", option.c_str());
      }
    }
}
...
{ if (unrecognized.size())
    raiseError(PyExc_ValueError, "unrecognized option '%s'", (*unrecognized.begin()).first.c_str());
}
...
{ TMultiStringParameters::const_iterator oi=options.find(par);
  if (oi==options.end())
    raiseError(PyExc_ValueError, "parameter '%s' not found", par.c_str());
  return (*oi).second;
}
...
bool readAnAtom(char *&curr, string &atom)
{
  ...
  if (*curr=='"') {
    ...
    if (*curr++!='"')
      raiseError(PyExc_ValueError, "newline in string '%s'", atom.c_str());
  }
  else if (*curr=='(') {
    int parnths=1;
    ...
    if (parnths)
      raiseError(PyExc_ValueError, "to many ('s in '%s'", string(start, curr).c_str());
...
{ char line[1024];
  ...
  if (str.gcount()==1024-1)
    raiseError(PyExc_ValueError, "line too long");
  return line;
}
      ...
      if ((*oi)[0]=='@') {
        ...
        if (*be=='@') {
          ...
          if (!pstr.is_open() || pstr.fail() || pstr.eof())
            raiseError(PyExc_ValueError, "invalid parameter file");
          ...
          if (!lineNo)
            raiseError(PyExc_ValueError, "Invalid line number (%s) for parameter file %s.", lineNos.c_str(), filename.c_str());
          ifstream pstr(filename.c_str());
          if (!pstr.is_open() || pstr.fail() || pstr.eof())
            raiseError(PyExc_ValueError, "Invalid parameter file (%s).", (*oi).c_str());
          while(--lineNo) {
            getSLine(pstr);
            if (pstr.fail() || pstr.eof())
              raiseError(PyExc_ValueError, "can't read parameter file %s to line %s", filename.c_str(), lineNos.c_str());
          }
          string nl=getSLine(pstr);
