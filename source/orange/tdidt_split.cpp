...
{ checkProperty(measure);
  ...
  TDiscreteVariable *evar = attribute.AS(TDiscreteVariable);
  if (evar)
...
{ 
  ...
  if (measure->needs==TMeasureAttribute::Generator) {
    ...
    if (subsetSizes && subsetSizes->variable)
      ...
      TDiscreteVariable *evar = mlnew TDiscreteVariable("");
      evar->addValue("0");
      evar->addValue("1");
      bvar = evar;
    }
...
//      raiseWarningWho("TreeSplitConstructor_ExhaustiveBinary", "this class is not optimized for 'candidates' list and can be very slow");
  ...
  if (attribute->noOfValues() == 2) {
    spentAttribute = bestAttr;
    descriptions = mlnew TStringList(attribute.AS(TDiscreteVariable)->values.getReference());
    return mlnew TClassifierFromVarFD(attribute, gen->domain, bestAttr, subsetSizes);
  }
...
{ 
  ...
  if (measure->needs==TMeasureAttribute::Generator) {
    ...
    if (subsetSizes && subsetSizes->variable)
      ...
      TDiscreteVariable *evar = mlnew TDiscreteVariable("");
      const string &value = gen->domain->attributes->at(bestAttr).AS(TDiscreteVariable)->values->at(bestValue);
  ...
  const string &bestValueS = attribute.AS(TDiscreteVariable)->values->at(bestValue);
  descriptions = mlnew TStringList();
  ...
  TIntList *bestMapping = mlnew TIntList(attribute.AS(TDiscreteVariable)->values->size(), 0);
