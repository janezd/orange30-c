...
{ 
  ...
  PyObject *res;
  bool ires;
  res = PyObject_RichCompare(value, other->value, Py_LT);
  if (!res)
	  throw pyexception();
  ires = (res == Py_True);
  Py_DECREF(res);
  ...
  res = PyObject_RichCompare(value, other->value, Py_GT);
  if (!res)
	  throw pyexception();
  ires = (res == Py_True);
  Py_DECREF(res);
...
{ 
  ...
  PyObject *res = PyObject_RichCompare(value, other->value, Py_EQ);
  if (!res)
    throw pyexception();
  bool comp = (res == Py_True);
  Py_DECREF(res);
  return comp;
}
...
{ 
  ...
  // the special format
  if (PyPythonValueSpecial_Check(pyvalue)) {
  ...
  if(!useSomeValue || !PyValue_Check(pyvalue))
...
void TPythonVariable::str2val(const string &valname, TValue &valu)
{
  ...
  if (isOverloaded("str2val"))
    ...
    valu = toValue(PyUnicode_FromString(valname.c_str()));
}
...
void TPythonVariable::val2filestr(const TValue &val, string &str, const TExample &example) const
{
  ...
  if (isOverloaded("val2filestr")) {
    ...
    if (usePickle) {
      ...
      if (!pickled)
        throw pyexception();
      if (!PyUnicode_Check(pickled)) {
        Py_DECREF(pickled);
        raiseError("cPickle.dumps returned a non-string(?!)");
      }

      const char *pickleds = PyUnicode_AS_DATA(pickled);
      int newlines = 1;
      const char *pi = pickleds;
      char *ei;
      while(*pi)
        if (*pi++ == '\n')
          newlines++;
...
void TPythonVariable::filestr2val(const string &valname, TValue &valu, TExample &ex)
{
  ...
  *ui = 0;
  ...
  PyObject *fdoms = PyUnicode_FromString("__fileExample");
...
int TPythonVariable::noOfValues() const
{
  if (isOverloaded("__len__")) {
    ...
    if (!PyLong_Check(pynvalues))
      raiseError("PythonVariable.__len__ should return an integer");

    int res = (int)PyLong_AsLong(pynvalues);
    Py_DECREF(pynvalues);
    return res;
  }
