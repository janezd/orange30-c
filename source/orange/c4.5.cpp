...
void loadC45()
{
  ...
  if (orangeDirName) {
    string sodn = PyUnicode_As_string(orangeDirName);
    const char *odn = sodn.c_str();
...
{ 
  TDiscreteVariable *classVar=dom->classVar.AS(TDiscreteVariable);
  if (!classVar)
  ...
  PITERATE(TVarList, vi, dom->attributes) {
    ...
    *AttNamei = mlnew char[(*vi)->name.length()+1];
    ...
    if ((*vi)->varType==TValue::INTVAR) {
      int noOfValues = (*vi).AS(TDiscreteVariable)->noOfValues();
      ...
      *(AttValNameii++)=NULL;
      PITERATE(TStringList, ni, (*vi).AS(TDiscreteVariable)->values) {
...
{ 
  ...
  int i, e;
  for(i = 0, e = domain->classVar.AS(TDiscreteVariable)->values->size(); i!=e; i++, cd++)
  ...
  if (nodeType == Subset) {
    int ve = tested.AS(TDiscreteVariable)->values->size();
...
TValue TC45Classifier::operator ()(const TExample &oexample)
{
  ...
  for(int cl = 0, ce = classVar.AS(TDiscreteVariable)->values->size(); cl!=ce; cl++, pi++) {
    if (*pi > bestP) {
      bestP = *pi;
      bestClass = cl;
    }
...
void TC45Classifier::predictionAndDistribution(const TExample &oexample, TValue &value, PDistribution &dist)
{
  ...
  int bestClass = 0;
  float bestP = -1;
  for(int cl = 0, ce = classVar.AS(TDiscreteVariable)->values->size(); cl!=ce; cl++) {
...
{ 
  ...
  TDiscreteVariable *dummyVar = mlnew TDiscreteVariable(tested->name);
  ...
  switch (nodeType) {
    case Branch: {
      ...
      newNode->branchDescriptions = mlnew TStringList(tested.AS(TDiscreteVariable)->values.getReference());
      break;
    }
    ...
    case Subset: {
      ...
      TStringList::const_iterator tvi(tested.AS(TDiscreteVariable)->values->begin());
