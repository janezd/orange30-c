...
{ PyTRY
    ...
    try {
      ...
      if (!iterator) {
        PyErr_Format(PyExc_TypeError, "'%s.__cmp__': not a sequence", Py_TYPE(self)->tp_name);
        return -1;
      }
      ...
      for(TValueList::iterator ii(aList->begin()), ei(aList->end()); ii!=ei; ii++) {
		...
		const int greater = PyObject_RichCompareBool(myItem, hisItem, Py_GT);
		if (greater) {
			Py_DECREF(myItem);
			Py_DECREF(hisItem);
			Py_DECREF(iterator);
		...
		const int smaller = PyObject_RichCompareBool(myItem, hisItem, Py_LT);
		Py_DECREF(myItem);
		Py_DECREF(hisItem);
		myItem = NULL; // prevent deallocation at exception handler below
		hisItem = NULL;
		if (smaller) {
			Py_DECREF(iterator);
			return -1; // smaller or error, we return -1 in either case
		}
      }
...
PyObject *TValueListMethods::_filter(TPyOrange *self, PyObject *args)
{
  PyTRY
    ...
    PyObject *newList = Py_TYPE(self)->tp_new(Py_TYPE(self), emtuple, emdict);
    Py_DECREF(emtuple);
    Py_DECREF(emdict);
    emtuple = NULL;
    emdict = NULL;
...
{ PyTRY
    ...
    for(iterator bi=aList->begin(), be=aList->end(); bi!=be; bi++)
      if (obj==*bi)
        return PyLong_FromLong(bi - aList->begin());
...
{ if (cmpfunc) {
    ...
    int res = (int)PyLong_AsLong(cmpres);
    Py_DECREF(cmpres);
