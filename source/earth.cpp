...
PClassifier TEarthLearner::operator() (PExampleTable const &examples)
{
	TDomain &domain = *examples->domain;
	int num_preds = domain.attributes->size();
	int num_cases = examples->size();
	int num_responses = 1;
...
//	num_cases = 100;
	...
	TExampleTable::iterator ex_iter = examples->begin();
	for (int i=0; i<num_cases; i++, ++ex_iter)
	{
		TExample::const_iterator value = ex_iter->begin();
		for (int j=0; j<num_preds; j++, value++)
		{
			x_(i, j) = isnan(*value) ? 0 : *value;
		}
		TValue const &class_value = ex_iter.getClass();
        // TODO: what to do if the class is missing?
        y[i] = class_value;
	}
...
//	}
	...
	PEarthClassifier classifier(new TEarthClassifier(examples->domain,
        best_set, dirs, cuts, betas,
        num_preds, num_responses, num_terms, max_terms));
...
TEarthClassifier::TEarthClassifier(PDomain const &_domain,
    bool *best_set, int *dirs, double *cuts, double *betas,
    int _num_preds, int _num_responses, int _num_terms, int _max_terms)
    : TClassifier(_domain)
{
	_best_set = best_set;
	_dirs = dirs;
    ...
    : TClassifier(other)
{
	raiseError(PyExc_NotImplementedError,
        "copying of EarthClassifiers is not implemented");
}
...
TValue TEarthClassifier::operator()(TExample const *const example)
{
	...
	if (classVar->varType == TVariable::Discrete)
		return std::max<double>(0.0, floor(y + 0.5));
	else
		return y;
}
...
double* TEarthClassifier::to_xvector(TExample const *const example)
{
...
{
	PFloatListList list(new TFloatListList());
	for (int i=0; i<max_terms; i++)
	{
		TFloatList * inner_list = new TFloatList();
		for(int j=0; j<num_preds; j++)
			inner_list->push_back(_dirs[i + j*max_terms]);
		list->push_back(PFloatList(inner_list));
	}
	return list;
}
...
{
	PFloatList list(new TFloatList());
...
void TEarthClassifier::save_model(TByteStream &buffer)
{
	buffer.write(max_terms);
	buffer.write(num_terms);
	buffer.write(num_preds);
	buffer.write(num_responses);
...
void TEarthClassifier::load_model(TByteStream & buffer)
{
	if (max_terms)
		raiseError(PyExc_SystemError, "Cannot overwrite a model");
...
TOrange *TEarthClassifier::__new__(PyTypeObject *type, PyObject *args, PyObject *kw)
{
    PDomain domain;
    TByteStream buf;
    if (!PyArg_ParseTuple(args, "O&O&:EarthClassifier",
                          &PDomain::argconverter, &domain,
                          &TByteStream::argconverter, &buf)) {
            return NULL;
    }
    PEarthClassifier classifier(new TEarthClassifier());
    classifier->load_model(buf);
    return classifier.getPtr();
}


PyObject *TEarthClassifier::__getnewargs__()
{
    TByteStream buf;
    save_model(buf);
    return Py_BuildValue("Oy#", domain.borrowPtr(), buf.buf, buf.length());
}


PyObject *TEarthClassifier::formatEarth()
{
    format_earth();
    Py_RETURN_NONE;
}
