...
{ DEFINE_TYPENAME
  ...
  if (x.size() != y.size())
    raiseError(PyExc_ValueError, "summult: lists have different sizes");
...
{ DEFINE_TYPENAME
  ...
  if (flist1.size() != flist2.size())
    raiseError(PyExc_ValueError, "sumdiffsquared: lists have different sizes");
...
{ DEFINE_TYPENAME
  if (!flist.size())
    raiseError(PyExc_ValueError, "geometricmean: empty list");
  ...
  if (mult<=0.0)
    raiseError(PyExc_ValueError, "geometricmean: non-positive product");
...
{ DEFINE_TYPENAME
  if (!flist.size())
    raiseError(PyExc_ValueError, "harmonicmean: empty list");

  T sum=0.0;
  const_SITERATE(fi, flist)
    if (*fi==0.0)
      raiseError(PyExc_ZeroDivisionError, "harmonicmean: division by zero");
    else sum+=T(1.0) / *fi;
...
{ DEFINE_TYPENAME
  if (!flist.size())
    raiseError(PyExc_ValueError, "mean: empty list");
...
{ if (!med.size())
    raiseError(PyExc_ValueError, "median: empty list");
...
{ DEFINE_TYPENAME
  ...
  if (!flist.size())
    raiseError(PyExc_ValueError, "mode: empty list");
...
{ DEFINE_TYPENAME
  if (!flist.size())
    raiseError(PyExc_ValueError, "moment: empty list");
...
{ T mom2=samplevar(flist);
  if (mom2==0.0)
    raiseError(PyExc_ValueError, "skewness: variation is 0.0");
...
{ DEFINE_TYPENAME
  if (!flist.size())
    raiseError(PyExc_ValueError, "samplevar: empty list");
...
{ DEFINE_TYPENAME
  if (flist.size()<2)
    raiseError(PyExc_ValueError, "samplevar: empty or one-element list");
...
{ int tocut=int(flist.size()*proportion);
  if (tocut*2>flist.size())
    raiseError(PyExc_ValueError, "trim proportion too large");
...
{ const T FPMIN(1.0e-30);
  ...
  raiseError(PyExc_ValueError, "gcf: a too large, ITMAX too small");
}
...
{ const int ITMAX=100;
  ...
  gln=gammln(a);
  if (x<=0.0)
    raiseError(PyExc_ValueError, "gser: negative x");
...
{ if ((x<0.0) || (a<=0.0))
    raiseError(PyExc_ValueError, "gammp: invalid arguments");
...
{   const int ITMAX = 200;
    ...
    raiseError(PyExc_ValueError, "betacf: a or b too big, or ITMAX too small.");
    return -1.0;
}
...
{ if ((x<0.0) || (x>1.0))
    raiseError(PyExc_ValueError, "betai: bad x");
...
{ DEFINE_TYPENAME
  ...
  if (flist1.size() != flist2.size())
    raiseError(PyExc_ValueError, "pearsonr: lists of different sizes");
...
{ if (flist1.size() != flist2.size())
    raiseError(PyExc_ValueError, "spearmanr: lists of different sizes");
...
{ raiseError(PyExc_NotImplementedError, "pointbiserialr: not implemented"); }


template<class T>
double kendalltau(const vector<T> &x, const vector<T> &y, double &probrs)
{ if (x.size() != y.size())
    raiseError(PyExc_ValueError, "kendaltau: lists of different sizes");
  if (!x.size())
    raiseError(PyExc_ValueError, "kendaltau: empty lists");
...
{ if (x.size() != y.size())
    raiseError(PyExc_ValueError, "ttest_rel: lists of different sizes");
  ...
  if (sd==0.0)
    raiseError(PyExc_ValueError, "ttest_rel: sd==0, can't divide");
...
{ int n=x.size();
  ...
  if (exp) {
    if (exp->size()!=n)
      raiseError(PyExc_ValueError, "chi_square: lists of different sizes");
    for(int i=0; i<n; i++)
...
{ if (!cont.size())
    raiseError(PyExc_ValueError, "chisquare2d: invalid contingency table");
...
{ 
  ...
  int n = cont[0].size();
  if ((n<2) || (k<2))
    raiseError(PyExc_ValueError, "anova_rel: invalid contingency table");
  ...
  for(typename vector<vector<T> >::const_iterator conti(cont.begin()), conte(cont.end()); conti!=conte; conti++) {
    if ((*conti).size() != n)
      raiseError(PyExc_ValueError, "anova_rel: number of subject is not the same in all groups");
...
T friedmanf(const vector<vector<T> > &cont, double &chi2, int &dfnum, int &dfden, double &prob)
{
  ...
  int N = cont[0].size();
  if ((N<2) || (k<2))
    raiseError(PyExc_ValueError, "friedmanf: invalid contingency table");
  ...
  for(typename vector<vector<T> >::const_iterator conti(cont.begin()), conte(cont.end()); conti != conte; conti++) {
    if ((*conti).size() != N)
      raiseError(PyExc_ValueError, "friedmanf: number of subject is not the same in all groups");
...
{ vector<T> both(x);
  ...
  if (sd==0)
    raiseError(PyExc_ValueError, "mannwhitneyu: empty group");
...
{ typedef typename map<T, U>::const_iterator mapiterator;
  ...
  switch (method) {
    ...
    case DISTRIBUTE_MINIMAL: {  // All original points plus some in between to fill up the quota
      T ineach = double(nPoints - points.size()) / double(points.size()-1);
      T inthis = T(0.0);
    ...
    case DISTRIBUTE_MAXIMAL: {  // Just as many points as allowed
      T ineach = double(points.size()) / double(nPoints); 
      T inthis = T(0.0);
    ...
    case DISTRIBUTE_FIXED: {
      ...
      double step = ppos.size()/double(nPoints-2);
      double up = 1.5;

      T x1 = T(0.0);
      T dx = T(0.0);
      result.push_back(*ppos.begin());
      for(set<double>::const_iterator pi(ppos.begin()), pe(ppos.end());;) {
        do {
          x1 = *pi;
          if (++pi==pe)
            break;
