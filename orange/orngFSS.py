...
def attsAbovethreshold(scores, threshold=0.0):
  ...
  Result: the first N attributes (without measures)
  """
  pairs = list(filter(lambda x, t=threshold: x[1] > t, scores))
...
def FilterAttsAboveThresh(data=None, **kwds):
  filter = FilterAttsAboveThresh_Class(*(), **kwds)
  if data: return list(filter(data))
  else: return filter
...
def FilterBestNAtts(data=None, **kwds):
  filter = FilterBestNAtts_Class(*(), **kwds)
  if data: return list(filter(data))
  else: return filter
...
def FilterRelief(data=None, **kwds):
  filter = FilterRelief_Class(*(), **kwds)
  if data: return list(filter(data))
  else: return filter
...
def FilteredLearner(baseLearner, examples = None, weight = None, **kwds):
  learner = FilteredLearner_Class(*[baseLearner], **kwds)
    ...
    while not stop:
        ...
        if len(domain.attributes)>=2:
            ...
            for attr in domain.attributes:
                newdomain = orange.Domain([x for x in domain.attributes if x!=attr], classVar)
            ...
            if bestStat:
                domain = orange.Domain([x for x in domain.attributes if x!=bestAttr], classVar)
...
def StepwiseLearner(examples = None, weightID = None, **argkw):
    sl = StepwiseLearner_Class(*(), **argkw)
    if examples:
