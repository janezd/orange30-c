...
def simplex_with_xnDBG(cache, xp, Star, d):
    ...
    r=[]
    xnd = [xp[i] for i in range(len(xp)-1) if i!=d] # vse koordinate, razen d-te
    xndz = xp[-1]
    for s in Star:
        for (p,j) in [(cache.points[i],i) for i in s]:
            pd = [p[i] for i in range(len(p)-1) if i!=d] # vse koordinate, razen d-te
...
def firstTriangle(cache, dimensions, progressCallback = None, **args):
    ...
    if not cache.stars:
        cache.stars = [star(x, tri) for x in range(npoints)]
    S = cache.stars

    if not cache.dts:        
        cache.dts = [min([ min([ dist(points[x][:-1],points[v][:-1]) for v in simplex if v!=x]) for simplex in S[x]])*.1 for x in range(npoints)]


    if progressCallback:
        nPoints = 100.0/npoints
        
    for x, (S, xp, dt, deltas) in enumerate(list(zip(cache.stars, points, cache.dts, cache.deltas))):
        for d in dimensions:
            xn = xp[:-1]
            DBG=0
...
def starRegression(cache, dimensions, progressCallback=None, **args):
    ...
    for x,(S,p) in enumerate(list(zip(cache.stars,points))):
        if S==[]:
...
def pca(self,PCAthreshold=.8):
    ...
    eigv = [(evals[i]/sum_evals,i) for i in range(len(evals))]
    eigv.sort()
    eigv.reverse()
    total = 0
    for i in eigv:
...
def Dpca(self):
    ...
    return
    if not self.deltas:
        self.deltas = [[None] * len(self.contAttributes) for x in range(len(self.data))]
    ...
    if not self.stars:
        self.stars = [star(x, tri) for x in range(npoints)]
    S = self.stars
...
def qing1D(cache, dimensions, progressCallback = None, **args):
    ...
    sosedi = [(p[i],star1D(i,p)) for i in range(lenp)]
    mini = []
    maxi = []
    ...
    ext.sort()
    elist = list(zip(mini1,[-1]*len(mini1)))+list(zip(maxi,[1]*len(maxi)))
    elist.sort()
    ekstremi = []
    if elist[0][1]==-1: # ce je najprej minimum
        ekstrem = min([(p[k][-1],k,-1) for k in range(maxi[0])]) # poiscemo pravi min med tockami do prvega maxa
    else:
        ekstrem = max([(p[k][-1],k,1) for k in range(mini1[0])]) # sicer poiscemo prvi max med tockami do prvega mina
    ekstremi += [ekstrem]
    for i in range(1,len(elist)-1):
        if elist[i][-1]==1:
            ekstrem = max([(p[k][-1],k,1) for k in range(elist[i-1][0],elist[i+1][0]+1)])
        else:
            ekstrem = min([(p[k][-1],k,-1) for k in range(elist[i-1][0],elist[i+1][0]+1)])
        ekstremi += [ekstrem]        

    if elist[-1][1]==-1: # ce je zadnji ekstrem minimum
        ekstrem = min([(p[k][-1],k,-1) for k in range(maxi[-1],lenp)]) # poiscemo pravi min med tockami do prvega maxa
    else:
        ekstrem = max([(p[k][-1],k,1) for k in range(mini1[-1],lenp)]) # sicer poiscemo prvi max med tockami do prvega mina
    ekstremi += [ekstrem]
...
def pade(data, attributes = None, method = tubedRegression, outputAttr = -1, threshold = 0, MQCNotation = False, derivativeAsMeta = False, differencesAsMeta = False, correlationsAsMeta = False, originalAsMeta = False):
    ...
    cache.deltas = [[None] * len(cache.contAttributes) for x in range(len(data))]
    if method == tubedRegression:
        cache.errors = [[None] * len(cache.contAttributes) for x in range(len(data))]

    cache.nNeighbours = 30

    if not attributes:
        attributes = list(range(len(cache.contAttributes)))
