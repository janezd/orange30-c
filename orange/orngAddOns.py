...
import time
import socket
# urllib because we need 'urlretrieve'
# urllib2 because it reports HTTP Errors for 'urlopen'
...
def suggestVersion(currentVersion):
    ...
    try:
        xmlVerInt = list(map(int, currentVersion.split(".")))
    except:
        xmlVerInt = []
    verInt = list(map(int, version.split(".")))
...
class OrangeRegisteredAddOn():
    ...
    def prepare(self, id=None, name=42, version="auto", description=None, tags=None, authorOrganizations=None, authorCreators=None, authorContributors=None,
        ...
        import codecs
        xmlDoc.writexml(open(addonXmlPath, 'w', encoding="utf-8"))
...
class OrangeAddOnRepository:
    ...
    def clone(self, new=None):
        ...
        new.addOns = {}
        for (id, versions) in list(self.addOns.items()):
        ...
        if force or (self.lastRefreshUTC < time.time() - interval):            
            ...
            try:
                ...
                if protocol == "http": # A remote repository
                    ...
                    try:
                        repositoryXmlDoc = urllib.request.urlopen(self.url+"/addOnServer.py?machine=1")
                    ...
                    try:
                        repoConn = urllib.request.urlopen(self.url+'abc')
                        response = "".join(repoConn.readlines())
                    except Exception as e:
                        raise RepositoryException("Unable to load repository data: %s" % e)
                    addOnFiles = [x.split('"')[1] for x in re.findall(r'href\s*=\s*"[^"/?]*\.oao"', response)]
                    if len(addOnFiles)==0:
                        if firstLoad:
                    ...
                    for addOnFile in addOnFiles:
                        try:
                            addOnTmpFile = urllib.request.urlretrieve(self.url+"/"+addOnFile)[0]
...
def loadRepositories():
    ...
    if os.path.isfile(listFileName):
        try:
            ...
            availableRepositories = [repo.clone() for repo in pickle.load(file)]
            file.close()
        except Exception as e:
...
def saveRepositories():
    ...
    try:
        import pickle
        global availableRepositories
        pickle.dump(availableRepositories, open(listFileName, 'wb'))
    except Exception as e:
...
def addAddOnDirectoriesToPath():
    ...
    for addOn in list(installedAddOns.values()) + registeredAddOns:
...
def installAddOnFromRepo(addOnInRepo, globalInstall=False, refresh=True):
    try:
        tmpFile = urllib.request.urlretrieve(addOnInRepo.repository.url+"/"+addOnInRepo.fileName)[0]
    except Exception as e:
