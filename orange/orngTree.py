...
def replaceCcont(strg, mo, node, parent, tree):
    ...
    op = __opdict[op]
    num = float(num)
    N = sum([x[1] for x in list(node.distribution.items()) if op(x[0], num)], 0.)
    if by:
        whom = byWhom(by, parent, tree)
        if whom and whom.distribution:
            byN = sum([x[1] for x in list(whom.distribution.items()) if op(x[0], num)], 0.)
            if byN > 1e-30:
                N /= byN
        else:
            return insertDot(strg, mo)
...
def extractInterval(mo, dist):
    ...
    if out:
        ...
        return [x for x in dist.items() if lop(x[0], lower) or hop(x[0], upper)]
    else:
        lop = lowin == "(" and operator.gt or operator.ge
        hop = upin == ")" and operator.lt or operator.le
        return [x for x in dist.items() if lop(x[0], lower) and hop(x[0], upper)]
