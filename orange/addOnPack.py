...
class AddOnPackDlg(QWizard):
    ...
    def initOptionalMetaDataPage(self):
        ...
        for listEdit in [self.eTags, self.eAAuthors, self.eAContributors, self.eAOrganizations]:
            listEdit.parseEntries = lambda control=listEdit: [entry for entry in map(lambda x: x.strip(), str(control.toPlainText()).split("\n")) if entry]
            listEdit.focusOutEvent = formatList
    
    
    def nameChangeCallback(self):
        if not self.preferredDirTouched:
            name = str(self.eName.text()).strip()
    ...
    def accept(self):
        ...
        rao.prepare(self.ao.id, self.ao.name, str(self.eVersion.text()), str(self.eDescription.toPlainText()), self.eTags.parseEntries(),
