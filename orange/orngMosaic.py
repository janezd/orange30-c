...
class orngMosaic:
    ...
    def evaluateProjections(self):
        ...
        try:
            ...
            if self.qualityMeasure == CN2_RULES:
                ...
                for key in list(list(self.dictResults.keys())):
    ...
    def _Evaluate(self, attrs):
        ...
        if self.qualityMeasure in [CHI_SQUARE]:
            dists = []
            for attr in attrs:
                if attr not in self.attributeDistributions:
    ...
    def argumentation_Combining(self):
        if not self.arguments or not list(self.arguments.values()): return
        ...
        for classValue in self.arguments.keys():
            ...
            for count in [4,3,2]:
                ...
                for val, attrs, splits in candidates:
                    ...
                    for split in splits:
                        for part in split:
                            if tuple(part) in argList[len(part)]:
            ...
            indicesToKeep = [val[2] for val in list(argList[1].values()) + list(argList[2].values()) + list(argList[3].values()) + list(argList[4].values())]
    ...
    def argumentation_SemiNaive(self):
        ...
        for classValue in list(self.arguments.keys()):
            # create a dict for arguments of different lengths. each dict has arguments of this length and corresponding scores
            argList = {1: {}, 2: {}, 3:{}, 4:{}}
            arguments = self.arguments[classValue]
            for i in range(len(arguments)):
            ...
            for count in [4,3,2]:
                for key in list(argList[count].keys()):
                    eps, Nab = argList[count][key][0]
                    v = float(4*eps*eps*Nab)
                    if v == 0 or 1/v > 1 - self.clsTau:
                        argList[count].pop(key)

            indicesToKeep = [val[1] for val in list(argList[1].values()) + list(argList[2].values()) + list(argList[3].values()) + list(argList[4].values())]
    ...
    def evaluateAttributeOrder(self, attrs, valueOrder, conditions, revert, domain = None):
        ...
        while triedIndices[0] < maxVals[0]:
            ...
            combVal = reduce(operator.concat, list(map(operator.concat, [vals[i] for i in revert], ["-"]*len(vals))))[:-1]
    ...
    def findOptimalAttributeOrder(self, attrs, optimizeValueOrder = 0):
        ...
        for key in list(cont.keys()):
            if dist[key] == 0: conditions[key] = (0, 0)
                ...
                quotients = list(map(operator.div, cont[key], apriori))
        ...
        attrPerms = orngVisFuncts.permutations(list(range(len(attrs))))
        ...
        for attrPerm in attrPerms:                      # for all attribute permutations
            ...
            for order in possibleOrders:                # for all permutations of attribute values
                ...
                val = self.evaluateAttributeOrder(currAttrs, currValueOrder, conditions, list(map(attrPerm.index, list(range(len(attrPerm))))), domain)
    ...
    def load(self, filename, ignoreCheckSum = 0):
        ...
        if not ignoreCheckSum and "dataCheckSum" in settings and settings["dataCheckSum"] != self.data.checksum():
            if self.__class__.__name__ != "orngMosaic":
                from PyQt4.QtGui import QMessageBox
...
class SplitConstructor_MosaicMeasure(orange.TreeSplitConstructor):
    ...
    def __call__(self, gen, weightID, contingencies, apriori, candidates, nodeClassifier):
        ...
        dist = list(orange.Distribution(newFeature, gen).values())
...
class CartesianClassifier(orange.Classifier):
    ...
    def classifyExample(self, ex, what = orange.Classifier.GetValue):
        ...
        classValue = list(classDist.keys())[list(classDist.values()).index(max(classDist.values()))]
