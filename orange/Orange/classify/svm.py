...
class SVMClassifierWrapper(orange.SVMClassifier):
    ...
    def __init__(self, wrapped):
        ...
        for name, val in list(wrapped.__dict__.items()):
    ...
    def __reduce__(self):
        return SVMClassifierWrapper, (self.wrapped,), dict([(name, val) for name, val in list(self.__dict__.items()) if name not in self.wrapped.__dict__])
...
def getLinearSVMWeights(classifier, sum=True):
    ...
    classSV=dict([(value, [sv for sv in classifier.supportVectors if sv.getclass()==value]) for value in classes])
    ...
    for i in range(len(classes)-1):
        for j in range(i+1, len(classes)):
            w={}
            coefInd=j-1
            for svInd in range(*svRanges[i]):
                for attr in SVs.domain.attributes+list(SVs[svInd].getmetas(False, orange.Variable).keys()):
                    if attr.varType==orange.VarTypes.Continuous:
                        updateWeights(w, attr, to_float(SVs[svInd][attr]), classifier.coef[coefInd][svInd])
            coefInd=i
            for svInd in range(*svRanges[j]):
                for attr in SVs.domain.attributes+list(SVs[svInd].getmetas(False, orange.Variable).keys()):
                    if attr.varType==orange.VarTypes.Continuous:
                        updateWeights(w, attr, to_float(SVs[svInd][attr]), classifier.coef[coefInd][svInd])
            weights.append(w)
    ...
    if sum:
        ...
        for w in weights:
            for attr, wAttr in list(w.items()):
                scores[attr] += wAttr**2
        for key in scores:
...
def exampleWeightedSum(example, weights):
    sum=0
    for attr, w in list(weights.items()):
...
class SparseLinKernel(object):
    def __call__(self, example1, example2):
        ...
        """
        s=set(list(example1.getmetas().keys())+list(example2.getmetas().keys()))
        sum=0
...
class MeasureAttribute_SVMWeights(orange.MeasureAttribute):
    ...
    def __call__(self, attr, examples, weightId=None):
        if examples is self._cached_examples:
            ...
            weights = dict(list(zip(examples.domain.attributes, weights)))
...
class RFE(object):
    ...
    def __call__(self, data, numSelected=20, progressCallback=None):
        ...
        scores = sorted(list(scores.items()), key=itemgetter(1))
...
def exampleTableToSVMFormat(examples, file):
    ...
    """
    attrs = examples.domain.attributes + list(examples.domain.getmetas().values())
