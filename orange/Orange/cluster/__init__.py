...
def compute_bic(km):
	...
	for x, midx in zip(data, mapping):
...
def plot_silhouette(km, filename='tmp.png', fast=False):
    ...
    csizes = list(list(map(len, scores)))
    cpositions = [sum(csizes[:i]) + (i+1)*3 + csizes[i]/2 for i in range(km.k)]
    scores = reduce(lambda x,y: x + [0]*3 + sorted(y), scores, [])
    plt.barh(list(list(range(len(scores)))), scores, linewidth=0, color='c')
    plt.yticks(cpositions, list(list(map(str, list(list(range(km.k)))))))
...
def kmeans_init_random(data, k, _):
    ...
    :type distfun: :class:`orange.ExamplesDistance`
     """
    return data.getitems(random.sample(list(range(len(data))), k))
...
def orderLeaves(tree, matrix, progressCallback=None):
    ...
    tree.mapping.setattr("objects", list(range(len(tree))))
    M = {}
    ordering = {}
    visitedClusters = set()
            ...
            for u in Vl:
                for w in Vr:
                    if True: #Improved search
                        ...
                        def MFunc(xxx_todo_changeme):
                            (m, k) = xxx_todo_changeme
...
##            print "error 5:", k, w, list(right)
    ...
    u, w = min([(u, w) for u in tree.left for w in tree.right], key=lambda u_w: M[tree, u_w[0], u_w[1]])
...
class TablePlot(Table):
    ...
    def _update_positions(self, renderer):
        keys = numpy.array(list(self._cells.keys()))
    ...
    def _get_column_widths(self, renderer):
        ...
        for (row, col), cell in list(self._cells.items()):
...
#        print self.renderer.transform()
        if self.data:
            ...
            for i, ii in enumerate(self.tree):
                ex = self.data[ii]
                for j, jj in enumerate((self.attr_tree if self.attr_tree else list(range(len(self.data.domain.attributes))))):
