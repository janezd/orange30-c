...
def readCategories(silent=False):
    ...
    try:
        ...
        cats = pickle.load(cacheFile)
        try:
            version = pickle.load(cacheFile)
        except EOFError:
            version = 0
    ...
    for addOn in list(orngAddOns.installedAddOns.values()) + orngAddOns.registeredAddOns:
    ...
    pickle.dump(categories, cacheFile)
    pickle.dump(currentCacheVersion, cacheFile)
    if splashWindow:
        splashWindow.hide()
