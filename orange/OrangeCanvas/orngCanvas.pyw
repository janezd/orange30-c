...
import sys, os, pickle, orngRegistry, OWGUI
...
#        self.lineColor = QColor(*self.settings["lineColor"])
        ...
        if "WidgetTabs" not in self.settings or self.settings["WidgetTabs"] == []:
            ...
            for i in range(len(defaultTabs)-1,0,-1):
    ...
    def readShortcuts(self):
        ...
        if os.path.exists(shfn):
            for t in file(shfn).readlines():
                ...
                if cat in self.widgetRegistry and widgetName in self.widgetRegistry[cat]:
...
#        self.menuOptions.addAction( "Channel preferences",  self.menuItemPreferences)
    ...
    def readRecentFiles(self):
        ...
        if "RecentFiles" not in self.settings: return
    ...
    def addToRecentMenu(self, name):
        recentDocs = []
        if "RecentFiles" in self.settings:
            recentDocs = self.settings["RecentFiles"]
...
#                       "widgetSelectedColor": (0, 255, 0), "widgetActiveColor": (0, 0, 255), "lineColor": (0, 255, 0),
        ...
        try:
            ...
            self.settings.update(pickle.load(open(filename, "rb")))
        except:
            pass

        if "style" not in self.settings:
    ...
    def getWidgetIcon(self, widgetInfo):
        if widgetInfo.icon in self.iconNameToIcon:
