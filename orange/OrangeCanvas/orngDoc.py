...
import pickle, math, orngHistory
...
#        self.signalManager.setFreeze(0, outWidget.instance)
    ...
    def save(self, filename = None):
        ...
        open(filename, "wt").write(xmlText)
        doc.unlink()
    ...
    def loadDocument(self, filename, caption = None, freeze = 0):
        ...
        try:
            ...
            for widget in widgets.getElementsByTagName("widget"):
                ...
                settings = pickle.loads(settingsDict[widget.getAttribute("caption")])
...
import sys, os, pickle, orange, orngSignalManager, OWGUI
...
class GUIApplication(OWBaseWidget):
    ...
    def createWidget(self, fname, iconName, caption, shown, signalManager):
        widgetSettings = pickle.loads(self.strSettings[caption])
        m = __import__(fname)
    ...
    def loadSettings(self):
        try:
            file = open("%s", "rb")
            self.strSettings = pickle.load(file)
            file.close()
...
if __name__ == "__main__":
    ...
    def dumpWidgetVariables(self):
        for widget in self.widgets:
            ...
            v = list(vars(widget.instance).keys())
            v.sort()
            for val in v:
    ...
    def keyPressEvent(self, e):
        ...
        pressed = "-".join([_f for _f in [int(e.modifiers()) & x and y for x, y in [(Qt.ControlModifier, "Ctrl"), (Qt.AltModifier, "Alt")]] if _f] + [chr(e.key())])
