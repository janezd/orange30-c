...
class WidgetButton(QFrame, WidgetButtonBase):
    def __init__(self, tab, name, widgetInfo, widgetTabs, canvasDlg, buttonType = 2, size=30):
        ...
        if self.buttonType == WB_TABBAR_TEXT:
            numSpaces = buttonName.count(" ")
            if numSpaces == 1: buttonName = buttonName.replace(" ", "<br>")
            elif numSpaces > 1:
...
class WidgetListBase:
    ...
    def createWidgetTabs(self, widgetTabList, widgetRegistry, widgetDir, picsDir, defaultPic):
        ...
        for (tabName, show) in widgetTabList + extraTabs:
            if not show or tabName not in widgetRegistry: continue
...
#        self.treeWidget.setRootIsDecorated(0) 
    ...
    def insertWidgetTab(self, name, show = 1):
        ...
        for i in range(len(path)):
            ...
            if not show:
                item.setHidden(1)
            if "treeItemsOpenness" in self.canvasDlg.settings and fullName in self.canvasDlg.settings["treeItemsOpenness"]:
                item.setExpanded(self.canvasDlg.settings["treeItemsOpenness"][fullName])
            elif "treeItemsOpenness" not in self.canvasDlg.settings and self.treeWidget.topLevelItemCount() == 1:
                item.setExpanded(1)
...
class CanvasPopup(QMenu):
    def __init__(self, canvasDlg):
        ...
        self.suggestDict = dict([(widget.name, widget) for widget in reduce(lambda x,y: x+y, [list(cat.values()) for cat in list(cats.values())], [])])
        self.suggestItems = [QListWidgetItem(self.canvasDlg.getWidgetIcon(widget), widget.name) for widget in list(self.suggestDict.values())]
        ...
        for widget in widgets:
            if ioClasses == None:
                ...
                for category, show in self.canvasDlg.settings["WidgetTabs"]:
                    if not show or category not in self.canvasDlg.widgetRegistry:
                        continue
...
def constructCategoriesPopup(canvasDlg):
    ...
    for category, show in canvasDlg.settings["WidgetTabs"]:
        if not show or category not in canvasDlg.widgetRegistry:
            continue
        path = category.split("/")
