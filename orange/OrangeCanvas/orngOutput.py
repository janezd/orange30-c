...
def thread_safe_discard(func):
    ...
    def safe_wrapper(self, *args, **kwargs):
        ...
        if QThread.currentThread() is not self._thread_safe_thread:
            print("Calling", func, "from the wrong thread.", "Discarding the call!", file=sys.stderr)
        else:
...
def thread_safe_queue(func):
    ...
    def safe_wrapper(self, *args, **kwargs):
        if not hasattr(self, "_thread_safe_queue"):
            self._thread_safe_thread = self.thread()
        if QThread.currentThread() is not self._thread_safe_thread:
            print("Calling", func, "from the wrong thread.", "Queuing the call!", file=sys.stderr)
...
#        self.textOutput.zoomIn(1)
        ...
        w = h = 500
        if "outputWindowPos" in canvasDlg.settings:
    ...
    def showEvent(self, ce):
        ...
        if "outputWindowPos" in settings:
