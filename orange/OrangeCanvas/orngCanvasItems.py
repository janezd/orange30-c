...
class CanvasWidget(QGraphicsRectItem):
    ...
    def updateWidgetState(self):
        ...
        iconNum = sum([list(widgetState.get("Info", {}).values()) != [],  list(widgetState.get("Warning", {}).values()) != [], list(widgetState.get("Error", {}).values()) != []])
        ...
        if self.canvasDlg.settings["ocShow"]:        # if show icons is enabled in canvas options dialog
            ...
            off  = 0
            if len(list(widgetState.get("Info", {}).values())) > 0 and self.canvasDlg.settings["ocInfo"]:
                off  = self.updateWidgetStateIcon(self.infoIcon, startX, yPos, widgetState["Info"])
            if len(list(widgetState.get("Warning", {}).values())) > 0 and self.canvasDlg.settings["ocWarning"]:
                off += self.updateWidgetStateIcon(self.warningIcon, startX+off, yPos, widgetState["Warning"])
            if len(list(widgetState.get("Error", {}).values())) > 0 and self.canvasDlg.settings["ocError"]:
    ...
    def updateWidgetStateIcon(self, icon, x, y, stateDict):
        icon.setPos(x,y)
        icon.show()
        icon.setToolTip(reduce(lambda x,y: x+'<br>'+y, list(stateDict.values())))
