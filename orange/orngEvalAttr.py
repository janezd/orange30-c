...
class MeasureAttribute_MDLClass(orange.MeasureAttribute):
    def __call__(self, attr, data, aprioriDist = None, weightID = None):
        attrClassCont = orange.ContingencyAttrClass(attr, data)
        classDist = list(orange.Distribution(data.domain.classVar, data).values())
        nCls = len(classDist)
        nEx = len(data)
        priorMDL = logMultipleCombs(nEx, classDist) + logMultipleCombs(nEx+nCls-1, [nEx, nCls-1])
        postPart1 = [logMultipleCombs(sum(attrClassCont[key]), list(attrClassCont[key].values())) for key in list(attrClassCont.keys())]
        postPart2 = [logMultipleCombs(sum(attrClassCont[key])+nCls-1, [sum(attrClassCont[key]), nCls-1]) for key in list(attrClassCont.keys())]
        ret = priorMDL
