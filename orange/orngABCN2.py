        ...
        for cl_i,cl in enumerate(examples.domain.classVar):
            ...
            while aes:
                ...
                if rule:
                    ...
                    aes = [x for x in aes if not rule(x)]
                else:
                    aes = aes[1:]
            if not progress:
...
##        self.ruleFinder.evaluator.bestRule.quality = 0.8
    ...
    def sort_arguments(self, arg_examples, examples):
        ...
        if len(arg_examples)>0:
            ...
            sorted.sort(key=lambda x: x[self.argumentID].value.positiveArguments[0].quality, reverse=True)
...
class ABBeamFilter(orange.RuleBeamFilter):
    ...
    def __call__(self,rulesStar,examples,weightID):
        ...
        rulesStar.sort(key=lambda x: x.quality, reverse=True)
        argsNum=0
    ...
    def onlyPositives(self,rule):
        ...
        return list(map(operator.or_,ruleTab,self.argTab))==self.argTab
...
class ruleCoversArguments:
    ...
    def __call__(self, rule):
        ...
        for index in self.indices:
            if list(map(operator.or_,rule.filter.indices,index)) == rule.filter.indices:
                return True
        return False
