...
"""
    def __init__(self, width, height):
        ...
        import io
        self._svg = io.StringIO()
        self._defs = io.StringIO()
        self._gradients = {}
        
    def set_gradient(self, gradient):
        Renderer.set_gradient(self, gradient)
        if gradient not in list(self._gradients.items()):
    ...
    def get_fill(self):
        if self.render_state["gradient"]:
            return 'style="fill:url(#%s)"' % ([key for key, gr in list(self._gradients.items()) if gr == self.render_state["gradient"]][0])
        else:
