...
def statisticsByFolds(stats, foldN, reportSE, iterationIsOuter):
    ...
    if iterationIsOuter:
        ...
        stats = [x_fN for x_fN in list(zip(stats,foldN)) if x_fN[1]>0.0]
        stats = [ [x[lrn]/fN for x, fN in stats] for lrn in range(numberOfLearners)]
    else:
        stats = [ [x/Fn for x, Fn in [x_Fn for x_Fn in list(zip(lrnD, foldN)) if x_Fn[1] > 0.0]] for lrnD in stats]
...
def AP(res, reportSE = False, **argkw):
    ...
    if argkw.get("unweighted", 0) or not res.weights:
        for tex in res.results:
            APsByFold[tex.iterationNumber] = list(map(lambda res, probs: res + probs[tex.actualClass], APsByFold[tex.iterationNumber], tex.probabilities))
            foldN[tex.iterationNumber] += 1
    else:
        for tex in res.results:
            APsByFold[tex.iterationNumber] = list(map(lambda res, probs: res + probs[tex.actualClass] * tex.weight, APsByFold[tex.iterationNumber], tex.probabilities))
            foldN[tex.iterationNumber] += tex.weight
...
def BSS(res, **argkw):
    return [1-x/2 for x in BrierScore(*(res, ), **argkw)]
...
def Friedman(res, statistics, **argkw):
    ...
    for ri in splitByIterations(res):
        ranks = statc.rankdata(statistics(*(ri,), **argkw))
        if sums:
...
def rankDifference(res, statistics, **argkw):
    ...
    if k==2:
        return Wilcoxon(*(res, statistics), **argkw)
    else:
        return Friedman(*(res, statistics), **argkw)
...
def ROCslope(xxx_todo_changeme, xxx_todo_changeme1):
    (P1x, P1y, P1fscore) = xxx_todo_changeme
    (P2x, P2y, P2fscore) = xxx_todo_changeme1
    if (P1x == P2x):
        return 1e300
...
def TCverticalAverageROC(ROCcurves, samples = 10):
    def INTERPOLATE(xxx_todo_changeme2, xxx_todo_changeme3, X):
        (P1x, P1y, P1fscore) = xxx_todo_changeme2
        (P2x, P2y, P2fscore) = xxx_todo_changeme3
...
def AUC_iterations(AUCcomputer, iterations, computerArgs):
    ...
    for ite in iterations:
        ...
        if not foldsUsed:
            return aucs
        subsum_aucs = list(map(add, subsum_aucs, aucs))
    return subsum_aucs
            ...
            for classIndex2 in range(classIndex1):
                ...
                if subsum_aucs:
                    if method == 0:
                        ...
                        usefulClassPairs += 1
                    sum_aucs = list(map(add, sum_aucs, subsum_aucs))
    else:
...
def plotMcNemarCurve(file, allResults, proportions, legend, reference=-1):
    ...
    file.write("set multiplot\n\n")
    file.write("plot \\\n")
    tmap=list(range(reference))+list(range(reference+1, len(legend)))
    for i in tmap[:-1]:
        file.write("'-' title '%s' with linespoints pointtype %i,\\\n" % (legend[i], i+1))
...
def learningCurveLearners2PiCTeX(file, allResults, proportions, **options):
    return learningCurve2PiCTeX(*(file, allResults, proportions), **options)
...
def learningCurve2PiCTeX(file, allResults, proportions, **options):
    ...
    if "numberedx" in options:
        numberedx=options["numberedx"]
        if type(numberedx)==int:
            if numberedx>0:
...
def legendLearners2PiCTeX(file, learners, **options):
  return legend2PiCTeX(*(file, [orngMisc.getobjectname(learners[i], "Learner %i" % i) for i in range(len(learners))]), **options)
...
def graph_ranks(filename, avranks, names, cd=None, cdmethod=None, lowv=None, highv=None, width=6, textspace=1, reverse=False, **kwargs):
    ...
    if cd and cdmethod == None:
        ...
        def getLines(sums, hsd):
            ...
            def noLonger(xxx_todo_changeme4, notSig):
