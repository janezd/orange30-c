...
import os, re, urllib.request, urllib.parse, urllib.error, sys
import md5, pickle
...
class updateOrangeDlg(QMainWindow):
    ...
    def readInternetVersionFile(self, updateGroups = 1):
        try:
            f = urllib.request.urlopen(self.whatsupUrl)
        except IOError:
    ...
    def writeVersionFile(self):
        ...
        itms = list(self.downstuff.items())
        itms.sort(key=itemgetter(0))

        for g in self.dontUpdateGroups:
            vf.write("-%s\n" % g)
        for fname, (version, md) in itms:
            vf.write("%s=%s:%s\n" % (fname, reduce(lambda x,y:x+"."+y, [repr(x) for x in version]), md))
        vf.close()
    ...
    def executeUpdate(self):
        ...
        if self.settings["scripts"]:
            ...
            for fname, (version, location) in itms:
                ...
                if os.path.exists(fname) and fname in self.downstuff and self.downstuff[fname][0] < upstuff[fname][0]:      # there is a newer version
                    updatedFiles += self.updatefile(self.updateUrl + fname, location, version, self.downstuff[fname][1], "Updating")
                elif not os.path.exists(fname) or fname not in self.downstuff:
    ...
    def updatefile(self, webName, localName, version, md, type = "Downloading"):
        ...
        try:
            urllib.request.urlretrieve(webName, localName + ".temp", self.updateDownloadStatus)
