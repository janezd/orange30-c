...
#        QObject.connect(self.thread, SIGNAL("advance()"), lambda :(pb.setValue(pb.value()+1), master_pb.advance()))
    ...
    def EndDownload(self, exitCode=0):
        ...
        if str(exitCode) == "Ok":
            ...
            self.setData(2, Qt.DisplayRole, sizeof_fmt(float(self.size)))#QVariant(sizeof_fmt(float(self.size))))
            self.master.UpdateInfoLabel()
            self.UpdateToolTip()
        else:
            self.updateWidget.SetState(1)
            self.setData(2, Qt.DisplayRole, "Error occured while downloading:" + str(exitCode))#QVariant("Error occured while downloading:" + str(exitCode)))
...
#        QTimer.singleShot(50, self.UpdateFilesList)
    ...
    def HandleError(self, xxx_todo_changeme):
        (exc_type, exc_value, tb) = xxx_todo_changeme
        if exc_type >= IOError:
...
#        self.progressBarFinished()
    ...
    def SearchUpdate(self, searchString=None):
        strings = str(self.lineEditFilter.text()).split() #self.searchString.split() if searchString is None else unicode(searchString).split()
        tags = set()
        for item in self.updateItems:
