...
class AsyncCall(QObject):
    ...
    def execute(self):
        ...
        try:
            ...
            print("Exception in thread ", QThread.currentThread(), " while calling ", self.func, file=sys.stderr)
            self.emit(SIGNAL("finished(QString)"), repr(ex))
            self.emit(SIGNAL("finished(PyQt_PyObject, QString)"), self, repr(ex))
        ...
        self.emit(SIGNAL("finished(PyQt_PyObject, QString)"), self, "Ok")
