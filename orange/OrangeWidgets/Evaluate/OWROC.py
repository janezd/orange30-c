...
##            self.mergedConvexCKeys.append(QPen(Qt.black, 5))
    ...
    def updateCurveDisplay(self):
        ...
        showSomething = 0
        for cNum in range(len(list(zip(self.showClassifiers, self.mergedCKeys)))):
            ...
            ## 'None' averaging
            for iNum in range(len(list(zip(self.showIterations, self.classifierIterationCKeys[cNum], self.classifierIterationConvexCKeys[cNum])))):
...
##        self.update()
    ...
    def calcAverageCurves(self):
        ...
        for i, (isplit, show) in enumerate(list(zip(self.splitByIterations, self.showIterations))):
            if show:
        ...
        except (ValueError, SystemError) as er:
            print("Failed to compute vertical average ROC curve. " + er.message, file=sys.stderr)
...
#        self.update()
    ...
    def setCurveWidth(self, v):
        for cNum in range(len(list(zip(self.showClassifiers, self.mergedCKeys)))):
            ...
            for iNum in range(len(list(zip(self.showIterations, self.classifierIterationCKeys[cNum])))):
                self.classifierIterationCKeys[cNum][iNum].setPen(QPen(self.classifierColor[cNum], v))
        self.replot()
#        self.update()

    def setConvexCurveWidth(self, v):
        for cNum in range(len(list(zip(self.showClassifiers, self.mergedConvexCKeys)))):
            self.mergedConvexCKeys[cNum].setPen(QPen(self.classifierColor[cNum], v))
            for iNum in range(len(list(zip(self.showIterations, self.classifierIterationConvexCKeys[cNum])))):
