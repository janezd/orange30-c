...
class ClusterOptimization(OWBaseWidget):
    ...
    def evaluateClusters(self, data):
        ...
        for key in closureDict.keys():
            if key not in otherDict:
                if key in closureDict: closureDict.pop(key)
                if key in polygonVerticesDict: polygonVerticesDict.pop(key)
                if key in enlargedClosureDict: enlargedClosureDict.pop(key)
    ...
    def evaluatePointsInClusters(self):
        if self.clusterStabilityButton.isChecked():
            ...
            for i in range(len(self.allResults)):
                ...
                for key in list(self.allResults[i][VERTICES].keys()):
                    ...
                    indices = numpy.compress(validData, numpy.array(list(range(len(self.rawData)))), axis = 1)
    ...
    def load(self):
        ...
        if "parentName" in settings and settings["parentName"] != self.parentName:
...
def getNextPoints(graph, v, ns, closure):
    ...
    for (e,e2) in list(status.keys()):
        if (e,e2) not in status: continue
        if status[(e,e2)] == 0:
            ret.append((e,e2))
            for (f, f2) in list(status.keys()):
...
def addPointsToDict(dictionary, key, val):
    if key in dictionary: dictionary[key] = dictionary[key] + [val]
...
def getSortedClosurePoints(graph, closure):
    ...
    lists = []
    while list(pointDict.keys()) != []:
        for start in list(pointDict.keys()):
