...
#        self.graph.repaint()
    ...
    def s2nMixAnchorsAutoSet(self):
        ...
        if self.autoSetParameters:
            results = {}
            self.s2nSpread = 0
            permutations = orngVisFuncts.generateDifferentPermutations(list(range(len(self.graph.dataDomain.classVar.values))))
            for perm in permutations:
                ...
                for val in self.attrsNum:
                    ...
                    if list(results.keys()) != []: self.setStatusBarText("Current projection value is %.2f (best is %.2f)" % (acc, max(results.keys())))
                    else:                    self.setStatusBarText("Current projection value is %.2f" % (acc))

                    results[acc] = (perm, val)
            if list(results.keys()) == []: return
