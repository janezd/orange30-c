...
#        self.btnSendpie = OWGUI.button(self.controlArea, self, "Send Pie Charts", self.sendPies)
    ...
    def constructBar(self, distrib, numbs):
        ...
        totHeight = 0
        for j, num, dist in reversed(list(zip(list(range(len(numbs))), numbs, distrib))):
            ...
            for j, e in enumerate(ex[i:]):
                ...
                if s >= 0.99:
                    for ex2 in self.examples:
                        ...
                        self.selectedAttributes = list(range(i, j+i+1))
                        return
...
#            self.btnSendpie.setDisabled(True)
            ...
            hasName = None
            for metavar in [data.domain.getmeta(mykey) for mykey in list(data.domain.getmetas().keys())]:
