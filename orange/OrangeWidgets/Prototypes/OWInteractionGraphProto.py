...
class OWInteractionGraphProto(OWWidget):
    ...
    def initLists(self, data):
        ...
        for key in list(self.rectNames.keys()):
    ...
    def _setAttrVisible(self, name, visible = 1):
        if visible == 1:
            if name in list(self.rectNames.keys()): self.rectNames[name].show();
            self._showAttribute(name)
        else:
            if name in list(self.rectNames.keys()): self.rectNames[name].hide();
