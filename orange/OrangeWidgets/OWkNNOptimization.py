...
class OWVizRank(VizRank, OWWidget):
    ...
    def __init__(self, parentWidget = None, signalManager = None, graph = None, visualizationMethod = SCATTERPLOT, parentName = "Visualization widget"):
        ...
        if visualizationMethod != SCATTERPLOT:
            ...
            self.attributeCountCombo = OWGUI.comboBox(self.buttonBox, self, "attributeCount", items = list(range(3, 20)), tooltip = "Evaluate only projections with exactly (or maximum) this number of attributes", sendSelectedValue = 1, valueType = int, debuggingEnabled = 0)
...
#        OWGUI.comboBoxWithCaption(self.heuristicsSettingsContClassBox, self, "attrDiscContClass", "For discrete attributes:", items = [val for (val, m) in discMeasuresContClass], callback = self.removeEvaluatedAttributes)
        ...
        self.localOptimizationProjCountCombo = OWGUI.comboBoxWithCaption(self.localOptimizationSettingsBox , self, "locOptProjCount", "Number of best projections to optimize:           ", items = list(range(1,30)), tooltip = "Specify the number of best projections in the list that you want to try to locally optimize.\nIf you select 1 only the currently selected projection will be optimized.", sendSelectedValue = 1, valueType = int)
        ...
        self.localOptimizationProjMaxAttr    = OWGUI.comboBox(locOptBox, self, "locOptMaxAttrsInProj", items = list(range(3,50)), tooltip = "What is the maximum number of attributes in a projection?", sendSelectedValue = 1, valueType = int)
...
#        self.heuristicsSettingsNoClassBox.setVisible(not self.graph.dataHasClass)
        ...
        self.selectedClasses = list(range(len(self.graph.dataDomain.classVar.values)))
    ...
    def finishedAddingResults(self):
        ...
        if maxLen == -1: return
        if maxLen == 2: vals = [2]
        else: vals = list(range(3, maxLen+1))
    ...
    def updateGraph(self):
        ...
        for index in range(min(len(self.results), self.projectionCount)):
            ...
            for i in range(len(attrs)):
                for j in range(i+1, len(attrs)):
                    ...
                    # projection quality
                    if (Min, Max) not in bestDict:
                        bestDict[(Min, Max)] = self.results[index][self.ACCURACY]
            index += 1

        bestCount = max([1] + list(countDict.values()))
        ...
        for x in range(num):
            for y in range(num-x):
                ...
                if (attributes[x], attributes[yy]) in countDict:
                    countVal = countDict[(attributes[x], attributes[yy])]
                elif (attributes[yy], attributes[x]) in countDict:
                    countVal = countDict[(attributes[yy], attributes[x])]

                if (attributes[x], attributes[yy]) in bestDict:
                    accVal = bestDict[(attributes[x], attributes[yy])]
                elif (attributes[yy], attributes[x]) in bestDict:
...
class OWGraphAttributeHistogram(OWWidget):
    ...
    def updateGraph(self):
        ...
        for index in range(count):
            ...
            for attr in attrs:
                if attr not in attrCountDict:
                    attrCountDict[attr] = [0, {}]
                attrCountDict[attr][0] += val
                attrCountDict[attr][1][part] = attrCountDict[attr][1].get(part, 0) + val

        attrs = [(attrCountDict[key][0], attrCountDict[key][1], key) for key in list(attrCountDict.keys())]
        attrs.sort()
        attrs.reverse()
...
class OWGraphProjectionQuality(OWWidget):
    def __init__(self,parent=None, dialogType = VIZRANK_POINT, signalManager = None):
        ...
        OWGUI.comboBox(b3, self, "lineWidth", items = list(range(1,5)), callback = self.updateGraph, sendSelectedValue = 1, valueType = int)
...
##        butt.setMaximumWidth(60)
    ...
    def updateGraph(self):
        ...
        if self.sortProjections:
            ...
            indices.sort()
            classes = list(range(nrOfClasses)); classes.remove(cls); classes = [cls] + classes
        else:
            indices = [(i,i) for i in range(len(self.graphMatrix[0]))]
            classes = list(range(nrOfClasses))
