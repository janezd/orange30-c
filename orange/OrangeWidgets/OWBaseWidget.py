    ...
    def pyqtConfigure(obj, **kwargs):
        ...
        for name, val in kwargs.items():
            if meta.indexOfProperty(name) >= 0:
                obj.setProperty(name, val)#QVariant(val))
...
import sys, time, random, os, os.path, pickle, copy, orngMisc
import orange
    ...
    def loadSettings(self, file = None):
        ...
        if file:
            try:
                ...
                print("%s: Failed to load settings!\n%s" % (self.__class__.__name__, repr(ex)), file=sys.stderr)
                settings = None
