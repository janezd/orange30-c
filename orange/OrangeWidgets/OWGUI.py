...
class ControlledCallback:
    ...
    def acyclic_setattr(self, value):
        ...
        if self.f:
            if self.f in [int, float] and (not value or isinstance(value, str) and value in "+-"):
                value = self.f(0)
            else:
                value = self.f(value)
...
class CallBackListBox:
    ...
    def __call__(self, *args): # triggered by selectionChange()
        if not self.disabled and self.control.ogValue != None:
            ...
            while clist: # We used to use list.__delslice__ here, but it is no more in Py3k
                list.pop(clist)
...
class tableItem(QTableWidgetItem):
    def __init__(self, table, x, y, text, editType = None, backColor=None, icon=None, type = QTableWidgetItem.Type):
        ...
        self.setData(Qt.DisplayRole, text)#QVariant(text))        # we add it this way so that text can also be int and sorting will be done properly (as integers and not as text)
...
class TableBarItem(QItemDelegate):
    BarRole = next(OrangeUserRole) #Qt.UserRole + 1
    ...
    def paint(self, painter, option, index):
        ...
        if not isinstance(ratio, float):
            ...
            if isinstance(value, float) and getattr(self.widget, "showBars", False):
                col = index.column()
...
class IndicatorItemDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        rect = option.rect
        indicator = str(index.data(self.role))
        if isinstance(str, indicator) and indicator not in ("False", "None", ""):
            painter.save()
