...
class PyListModel(QAbstractListModel):
    ...
    def headerData(self, section, orientation, role=Qt.DisplayRole):
        if role == Qt.DisplayRole:
            return str(section)#QVariant(str(section))
    ...
    def data(self, index, role=Qt.DisplayRole):
        row = index.row()
        if role == Qt.DisplayRole:
            return self[index.row()]#QVariant(self[index.row()])
...
#        return VariableEditor(var, parent)
    ...
    def setModelData(self, editor, model, index):
        model.setData(index, editor.variable, Qt.EditRole)#QVariant(editor.variable), Qt.EditRole)
