...
class ClassificationTreeNode(GraphicsNode):
    def __init__(self, attr, tree, parent=None, parentItem=None, scene=None):
        ...
        self.majorityClass, self.majorityCount = max(self.tree.distribution.items(), key=lambda key_val: key_val[1])
...
def parseRules(rules):
    def joinCont(rule1, rule2):
        ...
        for i in [0,1]:
            if rule[i][1].startswith("in"):
                r=rule[i][1][2:]
                interval[i]=[r.strip(" ")[0]]+[float(a) for a in r.strip("()[] ").split(",")]+[r.strip(" ")[-1]]
            else:
                if "<" in rule[i][1]:
    ...
    rules.sort(key=lambda a: a[0].name)
    newRules=[rules[0]]
    for r in rules[1:]:
...
#        OWGUI.button(self.controlArea, self, "Save as", callback=self.saveGraph, debuggingEnabled = 0)
    ...
    def setNodeInfo(self, widget=None, id=None):
        ...
        if True:
            w = min(max([n.rect().width() for n in self.scene.nodes()] + [0]), self.MaxNodeWidth if self.LimitNodeWidth else sys.maxsize)
            for n in self.scene.nodes():
