    ...
    def drawAll(self, x, y):
        def getNearestAtt(selectedBeta):
            if isinstance(self.attribute, AttrLineCont):
                ...
                nearestLeft = filter(lambda x: x.betaValue == max([at.betaValue for at in [x for x in self.attribute.attValues if x.betaValue <= selectedBeta]]) ,self.attribute.attValues)[0]
                nearestRight = filter(lambda x: x.betaValue == min([at.betaValue for at in [x for x in self.attribute.attValues if x.betaValue >= selectedBeta]]) ,self.attribute.attValues)[0]
...
class AttValueMarker(QGraphicsEllipseItem):
    def __init__(self, attribute, canvas, z=50):
        self.r = 5
        QGraphicsEllipseItem.__init__(*(self,0,0,2*self.r,2*self.r,None,canvas))
...
class AttValue:
    ...
    def paint(self, canvas, rect, mapper):
        def errorCollision(line,z=SE_Z):
            col = [x for x in line.collidingItems() if x.zValue()==z]
            if len(col)>0:
                return True
            return False
...
##        self.label.setPlainText(self.name)
        ...
        for mar in range(len(markers_mapped)):
            ...
            if [x for x in self.selectValues if abs(x[0]-xVal)<4] == [] and xVal<max_mapped:
...
class BasicNomogramFooter(QGraphicsScene):
    ...
    def convertToPercent(self, atLine):
        ...
        percentList = [x for x in numpy.arange(0, maxPercent+0.1, 0.05) if x>minPercent and x<maxPercent]
...
#        self.resize(self.nomogram.pright, rect.height()+30)
    ...
    def updateMarkers(self):
        ...
        def getNearestAtt(selectedBeta, at):
            nearestLeft = [x for x in at.attValues if x.betaValue == max([v.betaValue for v in [x for x in at.attValues if x.betaValue <= selectedBeta]])][0]
            nearestRight = [x for x in at.attValues if x.betaValue == min([v.betaValue for v in [x for x in at.attValues if x.betaValue >= selectedBeta]])][0]
            return (nearestLeft, nearestRight)
...
class OWNomogramHeader(QGraphicsView):
    ...
    def mouseMoveEvent(self, ev):
        if self.scene():
            items = [ci for ci in self.scene().items(QPointF(ev.pos())) if ci.zValue()==50]
            if len(items)>0:
...
class OWNomogramGraph(QGraphicsView):
    ...
    def mousePressEvent(self, ev):
        ...
        if self.scene() and ev.button() == Qt.LeftButton:
            items = [ci for ci in self.scene().items(sc_pos) if ci.zValue()==50]
            if len(items)>0:
