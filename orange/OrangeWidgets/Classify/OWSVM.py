...
class OWSVM(OWWidget):
    ...
    def __init__(self, parent=None, signalManager=None, name="SVM"):
        ...
        b.layout().addWidget(OWGUI.doubleSpin(b, self, "C", 0.5, 512.0, 0.5, addToLayout=False, callback=lambda *x: self.setType(0), alignment=Qt.AlignRight),
                             0, 2)
        b.layout().addWidget(OWGUI.appendRadioButton(b, self, "useNu", "ν-SVM", addToLayout=False),
                             1, 0, Qt.AlignLeft)
        b.layout().addWidget(QLabel("Complexity bound (\u03bd)", b), 1, 1, Qt.AlignRight)
        ...
        self.kernelradio = OWGUI.radioButtonsInBox(b, self, "kernel_type", btnLabels=["Linear,   x∙y", "Polynomial,   (g x∙y + c)^d",
                    "RBF,   exp(-g|x-y|²)", "Sigmoid,   tanh(g x∙y + c)"], callback=self.changeKernel)
        ...
        eps = OWGUI.doubleSpin(b,self, "eps", 0.001, 0.5, 0.001, label="Numeric precision (ε)", labelWidth = 180, orientation="horizontal", alignment=Qt.AlignRight)
