        ...
        for at in range(len(att)):
            ...
            if att[at].varType == orange.VarTypes.Discrete:
                ...
                if self.data:
                    ...
                    catkeys = list(cl.conditionalDistributions[at].keys())
                    minAtValue, maxAtValue = catkeys[0], catkeys[-1]
                ...
                for i in range(2*numOfPartitions):
                    if curr_num+i*d>=minAtValue and curr_num+i*d<=maxAtValue:
                        ...
                        if self.data:
                            ...
                            thickness = 0.0
                        d_filter = [x for x in cl.conditionalDistributions[at].keys() if x>curr_num+i*d-d/2 and x<curr_num+i*d+d/2]
                        if len(d_filter)>0:
    ...
    def sortNomogram(self):
        def sign(x):
            if x<0:
                return -1;
            return 1;
        def compare_to_ordering_in_rules(x):
            return self.cl.rulesOrdering.index(x.name)
        def compare_to_ordering_in_data(x):
            return self.data.domain.attributes.index(self.data.domain[x.name])
        def compare_to_ordering_in_domain(x):
            return self.cl.domain.attributes.index(self.cl.domain[x.name])
        def compate_beta_difference(x):
            return -(x.maxValue-x.minValue)
        def compare_beta_positive(x):
            return -x.maxValue
        def compare_beta_negative(x):
            return x.minValue

        if not self.bnomogram:
            return
        if self.sort_type == 0 and hasattr(self.cl, "rulesOrdering"):
            self.bnomogram.attributes.sort(key=compare_to_ordering_in_rules)
        elif self.sort_type == 0 and self.data:
            self.bnomogram.attributes.sort(key=compare_to_ordering_in_data)
        elif self.sort_type == 0 and self.cl and self.cl.domain:
            self.bnomogram.attributes.sort(key=compare_to_ordering_in_domain)
        if self.sort_type == 1:
            self.bnomogram.attributes.sort(key=compate_beta_difference)
        elif self.sort_type == 2:
            self.bnomogram.attributes.sort(key=compare_beta_positive)
        elif self.sort_type == 3:
            self.bnomogram.attributes.sort(key=compare_beta_negative)
