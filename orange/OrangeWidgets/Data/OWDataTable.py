...
class ExampleTableModel(QAbstractItemModel):
    def __init__(self, examples, dist, *args):
        ...
        self.metas = list(self.examples.domain.getmetas().values())
        ...
        self.sorted_map = list(range(len(self.examples)))
#        self.showAttrLabels = True
    ...
    def data(self, index, role):
        ...
        if role == Qt.DisplayRole:
            ...
            if attr == self.classVar and col == len(domain.attributes) and domain.classVar: #check if attr is actual class or a duplication in the meta attributes
                return self.clsColor
            elif attr in self.metas:
                return self.metaColor
            ...
            dist = self.dist[col]
            return (dist.max - float(val)) / (dist.max - dist.min or 1)
    ...
    def rowCount(self, parent=QModelIndex()):
        if parent.isValid():
            ...
            return max([len(self.examples)] + [c[0] for c in self._other_data.keys()])
        
    def columnCount(self, index=QModelIndex()):
        return max([len(self.all_attrs)] + [c[1] for c in self._other_data.keys()])
    ...
    def sort(self, column, order=Qt.AscendingOrder):
        ...
        values = sorted(values, key=lambda t: t[0] if not t[0].isSpecial() else sys.maxsize, reverse=(order!=Qt.AscendingOrder))
...
class OWDataTable(OWWidget):
    ...
    def setTable(self, table, data):
        ...
        mo = list(data.domain.getmetas(True).items())
        if mo:
            mo.sort(key=lambda x: x[1].name.lower())
            metas.append(None)
