...
#        OWGUI.appendRadioButton(selMethRadio, self, "selectMethod", "Best ranked", insertInto=brBox, callback=self.selectMethodChanged)
    ...
    def setData(self,data):
        ...
        if self.data:
            self.usefulAttributes = [x for x in self.data.domain.attributes if x.varType in [orange.VarTypes.Discrete, orange.VarTypes.Continuous]]
    ...
    def discretizationChanged(self):
        ...
        for meas, cont in zip(self.measuresAttrs, self.handlesContinuous):
            if not cont and meas in self.measured:
    ...
    def reliefChanged(self):
        removed = False
        if "computeReliefF" in self.measured:
...
#        self.applyIf()
    ...
    def headerClick(self, index):
        ...
        if index < 2:
            ...
            self.sortBy = 3 + self.measuresShort.index(str(self.table.horizontalHeader().model().headerData(index, Qt.Horizontal)))
        self.sortingChanged()
        ...
        for attr in self.data.domain.attributes:
            if handlesContinuous or attr.varType == orange.VarTypes.Discrete:
                ...
                if not self.discretizedData:
                    ...
                    contAttrs = [attr for attr in self.data.domain.attributes if attr.varType == orange.VarTypes.Continuous]
                    at = []
                    attrDict = {}
    ...
    def reprint(self, noSort = False):
        ...
        for col, meas_idx in enumerate(self.selectedMeasures):
            ...
            values = [val for val in list(mdict.values()) if val != None]
            if values != []:
...
class RankItemDelegate(QItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if col < 2 or col-2 not in self.widget.minmax:        # we don't paint first two columns
            QItemDelegate.paint(self, painter, option, index)
            return
        ...
        if isinstance(value, float):        # in case we get "?" it is not ok
        ...
        self.drawDisplay(painter, option, option.rect, str(index.data(Qt.DisplayRole)))
        painter.restore()
