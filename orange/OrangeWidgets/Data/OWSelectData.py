...
class OWSelectData(OWWidget):
    ...
    def setData(self, data):
        ...
        if self.data:
            optmetas = list(self.data.domain.getmetas(True).values())
            optmetas.sort(key=lambda x: x.name)
            self.varList = self.data.domain.variables.native() + list(self.data.domain.getmetas(False).values()) + optmetas
    ...
    def setOutput(self):
        ...
        if self.data:
            ...
            matchingOutput = list(filter(self.data, 1))
            matchingOutput.name = self.data.name
            nonMatchingOutput = list(filter(self.data, 1, negate=1))
...
# but now it unchecks the checkbox as it is inserted 
    ...
    def updateInfoIn(self, data):
        ...
        """
        if data:
            varList = data.domain.variables.native() + list(data.domain.getmetas().values())
