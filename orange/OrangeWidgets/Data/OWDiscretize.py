...
class DiscGraph(OWGraph):
    ...
    def plotRug(self, noUpdate = False):
        ...
        if self.master.showRug:
            ...
            if self.contingency:
                ...
                freqhigh = []
                freqlow = list(self.probDist.items())
            else:
                return
    ...
    def plotProbCurve(self, noUpdate = False):
        ...
        if self.contingency and self.condProb and self.master.showTargetClassProb:
            xData = list(self.contingency.keys())[1:-1]
...
#        self.graph.setData(self.data)
    ...
    def customChanged(self, which):
        ...
        content = list(set(content))  # remove duplicates (except 8.0, 8.000 ...)
        try:
            content.sort(key=float)
        except:
            content = str(le.text())
        ...
        if self.originalData:
            ...
            if discType == 2:
                try:
                    ...
                    customs = list(set(float(x) for x in content))  # remove duplicates (except 8.0, 8.000 ...)
                    customs.sort()
                except:
                    customs = []
