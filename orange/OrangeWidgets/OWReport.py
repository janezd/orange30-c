...
#                self.javascript("newsel.scrollIntoView(document.getElementById('%s'));" % current.elementId)
    ...
    def saveReport(self):
        ...
        tt = open(self.indexfile, "rt").read()
...
##       so they have to be added here         
    ...
    def loadXML(self):
        ...
        filepref = "file:///"+self.tempdir
        if 1:#try:
            x = xml.dom.minidom.parse(open(str(filename)))
            x.normalize()
            entries = []
            files = []
            ...
            for fname, fdata in files:
                ...
                open(os.path.join(self.tempdir, fname), "wt").write(fdata)
...
def reportCell(item, tag, style):
    ...
    if isinstance(item, QTableWidgetItem):
        ...
        align = align if isinstance(align, int) else Qt.AlignLeft 
        alignment = {Qt.AlignLeft: "left", Qt.AlignRight: "right", Qt.AlignHCenter: "center"}.get(align & Qt.AlignHorizontal_Mask, "left")
        text = str(item.data(Qt.DisplayRole)).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        return '<%s style="%s; text-align: %s">%s</%s>' % (tag, style, alignment, text, tag)
        ...
        align = align if isinstance(align, int) else Qt.AlignLeft 
        alignment = {Qt.AlignLeft: "left", Qt.AlignRight: "right", Qt.AlignHCenter: "center"}.get(align & Qt.AlignHorizontal_Mask, "left")
        text = str(model.headerData(index, Qt.Horizontal, Qt.DisplayRole)).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        return '<%s style="%s; text-align: %s">%s</%s>' % (tag, style, alignment, text, tag)
