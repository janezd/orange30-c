            ...
            for attrY in range(attrX, len(self.data.domain)):
                ...
                for i in range(len(valsX)):
                    ...
                    for j in range(len(valsY)):
                        ...
                        actualCount = 0
                        try:
                            for val in contXY['%s-%s' %(list(contX.keys())[i], list(contY.keys())[j])]: actualCount += val
                        except: pass
                        self.probabilities['%s+%s:%s+%s' %(self.data.domain[attrX].name, list(contX.keys())[i], self.data.domain[attrY].name, list(contY.keys())[j])] = ((list(contX.keys())[i], valx), (list(contY.keys())[j], valy), actualCount, total)
                        self.probabilities['%s+%s:%s+%s' %(self.data.domain[attrY].name, list(contY.keys())[j], self.data.domain[attrX].name, list(contX.keys())[i])] = ((list(contY.keys())[j], valy), (list(contX.keys())[i], valx), actualCount, total)
