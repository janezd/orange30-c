...
class DisplayStatistics (QGraphicsScene):
    ...
    def displayStat(self, data, ind, dist):
        ...
        if not dist[ind] or not dist[ind].items():
            if not dist[ind]:
                attr_name.setPlainText("Widget cannot show distributions for attributes of this type.")
            else:
        ...
        if attr.varType == orange.VarTypes.Discrete:
            ...
            if len(attrDist) > 0 and max(attrDist) > 0:
                if self.parent.sorting == 0:
                    keys = list(attrDist.keys())
                else:
                    d = [(val, key) for (key, val) in list(attrDist.items())]
                    d.sort()
        ...
        if attr.varType == orange.VarTypes.Continuous:
            quartiles_list = reduce(lambda x, y: x+y, [[x[0]]*int(x[1]) for x in list(dist[ind].items())])
