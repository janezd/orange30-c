...
#                self.information(2, "Unused attribute values were removed.")
    ...
    def DrawData(self, attrList, x0x1, y0y1, side, condition, totalAttrs, usedAttrs = [], usedVals = [], attrVals = "", **args):
        (x0, x1) = x0x1
        (y0, y1) = y0y1
    ...
    def DrawText(self, side, attr, x0x1, y0y1, totalAttrs, usedAttrs, usedVals, attrVals):
        (x0, x1) = x0x1
        (y0, y1) = y0y1
        ...
        if not self.conditionalDict[attrVals]:
            ...
            if side in self.drawPositions: (x0, x1, y0, y1) = self.drawPositions[side]        # restore the positions where we have to draw the attribute values and attribute name
    ...
    def DrawLegend(self, data, x0x1, y0y1):
        (x0, x1) = x0x1
        (y0, y1) = y0y1
