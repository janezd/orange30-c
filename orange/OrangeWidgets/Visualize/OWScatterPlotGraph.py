...
#            return
        ...
        # slower, unoptimized drawing because we use different symbols and/or different sizes of symbols
            ...
            for i in range(len(self.rawData)):
                ...
                if (newColor, size, Symbol, showFilled) not in xPointsToAdd:
            ...
            if self.haveSubsetData:
                ...
                for i in range(len(self.rawSubsetData)):
                    ...
                    if (newColor, size, Symbol, 1) not in xPointsToAdd:
        ...
        if self.showLegend == 1:
            ...
            if shapeIndex != -1:
                ...
                if shapeIndex in legendKeys:  val = legendKeys[shapeIndex]
            ...
            if sizeIndex != -1:
                ...
                if sizeIndex in legendKeys:  val = legendKeys[sizeIndex]
        ...
        if legendKeys != self.oldLegendKeys:
            ...
            self.legend().clear()
            for val in list(legendKeys.values()):       # add new curve keys
...
##                self.addCurve("", color, color, 1, QwtPlotCurve.Lines, QwtSymbol.NoSymbol, xData = [float(shortData[p1][0]), float(shortData[p2][0])], yData = [float(shortData[p1][1]), float(shortData[p2][1])], lineWidth = width)
    ...
    def addTip(self, x, y, attrIndices = None, dataindex = None, text = None):
        ...
        if text == None:
            ...
            elif self.tooltipKind == ALL_ATTRIBUTES:    text = self.getExampleTooltipText(self.rawData[dataindex], list(range(len(self.attributeNames))))
