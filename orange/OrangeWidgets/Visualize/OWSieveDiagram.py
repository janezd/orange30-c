...
#                self.information(1, "Unused attribute values were removed.")
    ...
    def updateGraph(self, *args):
        ...
        for i in range(len(valsX)):
            ...
            for j in range(len(valsY)):
                ...
                actualProb = 0
                try:
                    for val in contXY['%s-%s' %(list(contX.keys())[i], list(contY.keys())[j])]: actualProb += val
                except:
                    actualProb = 0
                probs['%s-%s' %(list(contX.keys())[i], list(contY.keys())[j])] = ((list(contX.keys())[i], valx), (list(contY.keys())[j], valy), actualProb, len(data))
        ...
        for i in range(len(valsX)):
            for j in range(len(valsY)):
                ((xAttr, xVal), (yAttr, yVal), actual, sum) = probs['%s-%s' %(list(contX.keys())[i], list(contY.keys())[j])]
    ...
    def addRectIndependencePearson(self, rect, x, y, w, h, xAxV, yAyV, actual, sum):
        (xAttr, xVal) = xAxV
        (yAttr, yVal) = yAyV
        expected = float(xVal*yVal)/float(sum)
        pearson = (actual - expected) / sqrt(expected)
