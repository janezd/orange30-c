...
class OWLinProjGraph(OWGraph, orngScaleLinProjData):
    ...
    def updateData(self, labels = None, setAnchors = 0, **args):
        ...
        if setAnchors or ("XAnchors" in args and "YAnchors" in args):
        ...
        if self.showAnchors:
            ...
            shownAnchorData = list(filter(lambda p, r=self.hideRadius**2/100: p[0]**2+p[1]**2>r, self.anchorData))
        ...
        # ##############################################################
            ...
            for i in range(dataSize):
                ...
                if (newColor, curveSymbol,0) not in xPointsToAdd:
            ...
            for i in range(len(self.rawSubsetData)):
                ...
                if (newColor, curveSymbol, 1) not in xPointsToAdd:
        ...
        # ##############################################################
            ...
            for i in range(dataSize):
                ...
                if (newColor, curveSymbol, self.showFilledSymbols) not in xPointsToAdd:
    ...
    def addTooltipKey(self, x, y, color, index, extraString = None):
        dictValue = "%.1f-%.1f"%(x, y)
        if dictValue not in self.dataMap: self.dataMap[dictValue] = []
