...
##        box.layout().addWidget(self.colorPalette)
        ...
        try:
            ...
            print(ex, "Error loading saved color palettes!\nCreating new default palette!", file=sys.stderr)
            self.colorSettings = None
            self.colorCombo.setPalettes("palette", self.createColorDialog())
        ...
        if len(self.selection.getSelection())==0:
            ...
            for sel in selection:
                ...
                if (len(self.distanceMap.elementIndices)==0):
                    ...
                    tmp += list(range(self.distanceMap.elementIndices[x1], self.distanceMap.elementIndices[x2]))
                    tmp += list(range(self.distanceMap.elementIndices[y1], self.distanceMap.elementIndices[y2]))
        ...
        if (row==-1 or col==-1):
            ...
            if self.ShowBalloon == 1:
                ...
                if (self.ShowItemsInBalloon == 1):
                    ...
                    if not isinstance(namei, str):
                        namei = namei.name
                    if not isinstance(namej, str):
                        namej = namej.name
                    if namei or namej:
                        body = namei + "\n" + namej
                    else:
...
#        self.scene.update()
    ...
    def sortRandom(self):
        import random
        self.order = list(range(len(self.matrix.items)))
