...
class OWNetExplorer(OWWidget):
    ...
    def __init__(self, parent=None, signalManager=None):
        ...
        self.edgeColorSettings = [('net_edges', [[], [('contPalette', (4294967295, 4278190080, 0))], [('discPalette', [(204, 204, 204), (179, 226, 205), (253, 205, 172), (203, 213, 232), (244, 202, 228), (230, 245, 201), (255, 242, 174), (241, 226, 204)])]]), ('Default', [[], [('contPalette', (4294967295, 4278190080, 0))], [('discPalette', [(0, 0, 255), (255, 0, 0), (0, 255, 0), (255, 128, 0), (255, 255, 0), (255, 0, 255), (0, 255, 255), (128, 0, 255), (0, 128, 255), (255, 223, 128), (127, 111, 64), (92, 46, 0), (0, 84, 0), (192, 192, 0), (0, 127, 127), (128, 0, 0), (127, 0, 127)])]])]
    ...
    def nameComponents(self):
        ...
        def rank(a, j, reverse=False):                    
            ...
            if reverse:                
                ...
                a.sort(key=itemgetter(j), reverse=True)
                top_value = a[0][j]
                top_rank = len(a)
                max_rank = float(len(a))
                int_ndx = 0
        ...
        for i in range(len(components)):
            ...
            res = list(res1.items()) + list(res2.items())
    ...
    def nameComponents_old(self):
        ...
        for component in components:
            ...
            words = sorted(list(counted_words.items()), key=itemgetter(1), reverse=True)
            keyword = ""
            keyword_words = []
            max_count = 0
            i = 0
        ...
        for component in sameComponents:
            ...
            words = sorted(list(list(counted_words.items())), key=itemgetter(1), reverse=True)
            keyword = ""
    ...
    def setMarkMode(self, i = None):
        ...
        if hubs == 0:
            ...
            toMark = [i for i, values in enumerate(vgraph.items) if txt.lower() in " ".join([str(values[ndx]).decode("ascii", "ignore").lower() for ndx in list(range(len(vgraph.items.domain))) + list(vgraph.items.domain.getmetas().keys())])]
        ...
        if hubs == 4: # at least N connections
            ...
            #print "mark most"
            sortedIdx = list(range(len(powers)))
            sortedIdx.sort(key=lambda x: powers[x])
    ...
    def setCombos(self):
        ...
        for var in vars:
            if var.varType in [orange.VarTypes.Discrete, orange.VarTypes.Continuous]:
                self.colorCombo.addItem(self.icons[var.varType],var.name)
            ...
            if var.varType in [orange.VarTypes.String] and hasattr(self.optimization.graph, 'items') and self.optimization.graph.items != None and len(self.optimization.graph.items) > 0:
                ...
                try:
                    if type(eval(value)) == type([]):
                        self.vertexSizeCombo.addItem(self.icons[var.varType],var.name)
                        continue
                except:
                    pass
                
                if len(value.split(',')) > 1:
                    self.vertexSizeCombo.addItem(self.icons[var.varType], "num of " +var.name)
                
            elif var.varType in [orange.VarTypes.Continuous]:
                self.vertexSizeCombo.addItem(self.icons[var.varType],var.name)

            self.nameComponentCombo.addItem(self.icons[var.varType], var.name)
            self.showComponentCombo.addItem(self.icons[var.varType], var.name)
            self.editCombo.addItem(self.icons[var.varType], var.name)
            self.comboAttSelection.addItem(self.icons[var.varType], var.name)
        
        for var in edgeVars:
            if var.varType in [orange.VarTypes.Discrete, orange.VarTypes.Continuous]:
                self.edgeColorCombo.addItem(self.icons[var.varType], var.name)
            ...
            if len(commonVars) > 0:
                for var in commonVars:
                    ...
                    if orgVar.varType == mrkVar.varType and orgVar.varType == orange.VarTypes.String:
                        self.markInputCombo.addItem(self.icons[orgVar.varType], orgVar.name)
        ...
        if subset != None:
            try:
                ...
                for row in subset:
                    ...
                    if expected != index:
                        hiddenNodes += list(range(expected-1, index-1))
                        expected = index + 1
                    else:
                        expected += 1
                        
                hiddenNodes += list(range(expected-1, self.graph.nVertices))
