...
#        self.resize(100,100)
    ...
    def showResults(self):
        ...
        for i, (k, run) in enumerate(self.optimizationRun):
            ...
            item.setData(Qt.TextAlignmentRole, Qt.AlignCenter) # QVariant(Qt.AlignCenter))
            
            item = OWGUI.tableItem(self.table, i, 1, None)#" " if (k, run) == self.bestRun else "")
            item.setData(OWGUI.IndicatorItemDelegate.IndicatorRole, (k, ron) == self.bestRun) #QVariant((k, run) == self.bestRun))
#            item.setData(Qt.DecorationRole, QVariant(QIcon(os.path.join(os.path.dirname(OWGUI.__file__), "icons", "circle.png"))))
            ...
            item.setData(OWGUI.TableBarItem.BarRole, (bestScore - run.score) / ((bestScore - worstScore) or 1) * 0.95) # QVariant((bestScore - run.score) / ((bestScore - worstScore) or 1) * 0.95))
    ...
    def sendData(self, km=None):
        ...
        for ex, midx in zip(new, km.clusters):
            ex[aid] = midx
