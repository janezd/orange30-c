...
def readMatrix(fn, progress=None):
    ...
    if type(fn) != file and (os.path.splitext(fn)[1] == '.pkl' or os.path.splitext(fn)[1] == '.sym'):
        ...
        try:
            ...
            raise Exception("Matrix dimension expected in the first line")
        #print dim
        ...
        for li, lne in enumerate(fle):
            if li > dim:
                if not li.strip():
                    continue
                raise IndexError("File too long")
            spl = lne.split("\t")
            if labeled:
                labels.append(spl[0].strip())
                spl = spl[1:]
            if len(spl) > dim:
                raise IndexError("Line %i too long" % li+2)
...
#        OWGUI.appendRadioButton(ribg, self, "invertMethod", "1 / X", callback = self.setInvertMode)
    ...
    def relabel(self):
        ...
        if self.data == None and self.labels == None:
            matrix.setattr("items", list(range(matrix.dim)))
