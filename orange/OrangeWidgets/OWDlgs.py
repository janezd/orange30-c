            ...
            if isinstance(self.graph, QwtPlot):
                ...
                for (z, item) in sortedList:
                    ...
                    if item.__class__ in [QGraphicsRectItem, QGraphicsLineItem]:
                        ...
                        f.write("text(%f, %f, '%s'%s%s, color = %s, name = '%s', weight = '%s', alpha = %.3f)\n" % (item.x, maxy-item.y, str(item.toPlainText()).encode("utf-8"), vertAlign, horAlign, color, str(item.font().family()), weight, item.defaultTextColor().alpha()/float(255)))
