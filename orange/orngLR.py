...
def Univariate_LogRegLearner(examples=None, **kwds):
    learner = Univariate_LogRegLearner_Class(*(), **kwds)
    if examples:
...
class Univariate_LogRegLearner_Class(orange.Learner):
    ...
    def __call__(self, examples):
        ...
        classifiers = [LogRegLearner(orange.Preprocessor_dropMissing(examples.select(orange.Domain(x, examples.domain.classVar)))) for x in examples.domain.attributes]
        ...
        def createLogRegExampleTable(data, weightID):
            ...
            for at in data.domain.attributes:
                ...
                # v dataOrig, dataFinal in newData atributu "at" dodaj ee  eno  vreednost, ki ima vrednost kar  ime atributa +  "X"
                    ...
                    newDomain = orange.Domain([x for x in data.domain.attributes if x!=at]+[atNew,data.domain.classVar])
                    newDomain.addmetas(data.domain.getmetas())
                    newData = orange.ExampleTable(newDomain,data)
                    altData = orange.ExampleTable(newDomain,data)
                    for i,d in enumerate(newData):
...
##        print "prej", betas_ap
        ...
        positives = sum([x for x in betas_ap if x>=0])
        negatives = -sum([x for x in betas_ap if x<0])
        if not negatives == 0:
    ...
    def __call__(self, examples, weight=0):
        ...
        def createLogRegExampleTable(data, weightID):
            ...
            for at in data.domain.attributes:
                ...
                # v dataOrig, dataFinal in newData atributu "at" dodaj ee  eno  vreednost, ki ima vrednost kar  ime atributa +  "X"
                    atNew = orange.EnumVariable(at.name, values = at.values + [at.name+"X"])
                    newDomain = orange.Domain([x for x in origData.domain.attributes if x!=at]+[atNew,origData.domain.classVar])
    ...
    while not stop:
        ...
        if len(attr) >= 2:
            ...
            for at in attr:
                ...
                tempAttr = [x for x in attr if x!=at]
...
def StepWiseFSS_Filter(examples = None, **kwds):
    ...
    filter = StepWiseFSS_Filter_class(*(), **kwds)
    if examples is not None:
        return list(filter(examples))
    else:
        return filter
