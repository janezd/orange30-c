...
class NetworkOptimization(orangeom.NetworkOptimization):
    ...
    def mdsUpdateData(self, components, mds, callbackUpdateCanvas):
        ...
        if self.mdsType == MdsType.MDS:
            ...
            self.graph.coors[0][list(range(self.graph.nVertices))] =  x
            self.graph.coors[1][list(range(self.graph.nVertices))] =  y
...
class NetworkClustering():
    ...
    def labelPropagation(self, results2items=0, resultHistory2items=0):
        ...
        vertices = list(range(self.net.nVertices))
        labels = list(range(self.net.nVertices))
        lblhistory = []
        #consecutiveStop = 0
        for i in range(1000):
        ...
        if resultHistory2items:
            ...
            dom = orange.Domain(attrs, 0)
            # transpose history
            data = list(map(list, list(zip(*lblhistory))))
