...
class VizRank:
    ...
    def kNNClassifyData(self, table):
        ...
        results = testingMethods[self.testingMethod](*[[learner], table])
    ...
    def kNNComputeAccuracy(self, table):
        ...
        if self.evaluationAlgorithm == ALGORITHM_KNN or self.externalLearner:
            ...
            if self.useExampleWeighting and testTable.domain.classVar and testTable.domain.classVar.varType == orange.VarTypes.Discrete:
                ...
                results = testingMethods[self.testingMethod](*[[learner], (testTable, weightID)])
            else:
                results = testingMethods[self.testingMethod](*[[learner], testTable])
    ...
    def computeAccuracyFromResults(self, table, results):
        ...
        classes = self.selectedClasses or list(range(len(self.graph.dataDomain.classVar.values)))
    ...
    def selectNextAttributeSubset(self, minLength, maxLength):
        ...
        if self.attrCont == CONT_MEAS_S2NMIX or self.attrSubsetSelection == GAMMA_SINGLE:
            if "attrs" not in self.evaluationData:
            ...
            if self.attrSubsetSelection in [GAMMA_ALL, GAMMA_SINGLE]:
                ...
                combinations = orngVisFuncts.combinations(list(range(z)), u)
            ...
            if "attrs" not in self.evaluationData:
                ...
                for i in range(minLength, maxLength+1):
                    ...
                    if self.projOptimizationMethod != 0 or self.visualizationMethod == KNN_IN_ORIGINAL_SPACE:
                        permutationIndices[i] = [list(range(i))]
                    else:
                        permutationIndices[i] = orngVisFuncts.generateDifferentPermutations(list(range(i)))
...
##                sys.stderr.write("selectNextAttributeSubset " + str(permutationIndices.keys()) + "\n")
            ...
            if len(attributes) < u+1:
                ...
                if self.attrCont == CONT_MEAS_NONE and self.attrDisc == DISC_MEAS_NONE:
                    ...
                    list(map(list.append, combinations, [attributes[z]] * len(combinations)))     # append the z-th attribute to all combinations in the list
    ...
    def getAttributeSubsetUsingGammaDistribution(self, attrCount):
        ...
        projCountWidth = len(list(triedDict.keys())) / 1000
        ...
        if self.attrCont == CONT_MEAS_S2NMIX or self.attrSubsetSelection == GAMMA_SINGLE:
            ...
            for i in range(maxTries):
                ...
                while len(attrs) < min(attrCount, len(self.graph.dataDomain.attributes)):
                    ...
                    tried += 1
                attrs.sort()
                if tuple(attrs) not in triedDict and len(attrs) == attrCount:
            ...
            for i in range(maxTries):
                ...
                attrList.sort()
                if tuple(attrList) not in triedDict:
        ...
        if self.attrCont == CONT_MEAS_S2NMIX or self.attrSubsetSelection == GAMMA_SINGLE:
            ...
            if self.attrSubsetSelection == GAMMA_SINGLE:
                ...
                for c in range(10):
                    ...
                    if tuple(comb) not in usedPerms:
...
#            return 0
        ...
        if self.visualizationMethod == SCATTERPLOT:
            ...
            while len(projections) > 0:
                ...
                for (attrIndex, attr) in enumerate(attributes):
                    ...
                    testProjections = []
                    if (projection[0], attr) not in tempDict and (attr, projection[0]) not in tempDict: testProjections.append([projection[0], attr])
                    if (projection[1], attr) not in tempDict and (attr, projection[1]) not in tempDict: testProjections.append([attr, projection[1]])
            ...
            while len(projections) > 0:
                ...
                if self.locOptOptimizeProjectionByPermutingAttributes == 1 and self.projOptimizationMethod == 0:
                    ...
                    while failedConsecutiveTries < 5 and tries < 50:
                        ...
                        tries += 1
                        if str(newProj) in triedPermutationsDict:
                    ...
                    for (attrIndex, attr) in enumerate(attributes):
                        ...
                        if self.projOptimizationMethod != 0:
                            ...
                            for proj in testProjections:
                                proj.sort()
                                if str(proj) in newProjDict: continue
                        ...
                        # ordinary radviz projections
                            ...
                            for testProj in testProjections:
                                if str(testProj) in newProjDict: continue
    ...
    def load(self, name, ignoreCheckSum = 1, maxCount = -1):
        ...
        if "dataCheckSum" in settings and settings["dataCheckSum"] != self.graph.rawData.checksum():
    ...
    def evaluateProjections(self, qApp = None):
        ...
        for result in results:
            if self.dialogType == VIZRANK_POINT:
                ...
                validDataIndices = list(range(len(data)))
