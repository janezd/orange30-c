...
def delCached(data, name):
    if not data: return
    if getattr(data, "info", None) != None and name in data.info:
        del data.info[name]
...
class DataHintsCache(object):
    ...
    def set_hint(self, data, key, value, weight=1.0):
        attrs = data.domain.variables + list(data.domain.getmetas().values())
        for attr in attrs:    
    ...
    def get_weighted_hints(self, data, key):
        ...
        return sorted(list(weighted_hints.items()), key=lambda key:key[1], reverse=True)
