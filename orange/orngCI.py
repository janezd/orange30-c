  ...
  def __call__(self, data, weight):
    ...
    for attr in ordered:
      ...
      if len(newattr.values) < len(attr.values):
        newset = [x for x in data.domain.attributes if x!=attr]
...
def FeatureGenerator(examples = None, weightID = 0, **argkw):
  fm = FeatureGeneratorClass(*(), **argkw)
  if examples:
...
def StructureInducer(examples=None, weightID=0, **argkw):
  fm = StructureInducerClass(*(), **argkw)
  if examples:
      fm = fm(examples, weightID)
  return fm
...
def replaceWithInduced(attr, table):
  return table.select(list(filter(lambda x, b=list(attr.getValueFrom.boundset()): not b.count(x), table.domain.attributes))+[attr, table.domain.classVar])
